...
spi2flash #(.NO_I2C_REGS(NO_TRIM_REGS))   spi2flash();
...
wire  imeas_grp_ctl;
wire  [11:0] active_channel_num;
wire [15:0] imeas_reg_ctrl;
wire [7:0]  imeas_reg_ch;
wire [2:0]  imeas_reg_seq;
wire [7:0]  imeas_reg_rstval;
wire        imeas_int_clr;
wire        imeas_int_alarm_clr;
wire   [15:0]   imeas_ch0data;
wire   [15:0]   imeas_ch1data;
wire   [15:0]   imeas_ch2data;
wire            imeas_int_set;
wire            imeas_int_alarm_set;
...
(
  ...
  .dpstb_en(flash_dpstb_en), //from SPI
  .zmeas_en(zmeas_en), //from SPI
  .imeas_en(imeas_en), //from SPI
...
zmeas u_zmeas(
   ...
   .ecgcal_reg_ctrl(imeas_reg_ctrl[15:8]), //imeas_reg_ctrl_1 is not used in imeas. Using it for ECG CAL
   ...
   /*
   .Z_SEL1V5(Z_SEL1V5),  //1:0, config_output_voltage_range[1:0]
   //2:measure_calibrate
   //3. pga_gain
   .Z_ADC_EN(Z_ADC_EN),   //from SPI, then select 2 source, then output to ana
   .Z_SYS_CLK(Z_SYS_CLK),  //adc_clk
   .DDS(zmeas_DDS),  //rom
...
wire notch_filter_en;  //1 is enable
wire [1:0] gain_mul;
imeas u_imeas(
...
.notch_filter_en(notch_filter_en),   //1 is enable
.gain_mul(gain_mul),
.cic_rate(cic_rate),
  .check_pulse_pclk(check_pulse_pclk), //
 ...
 .ch0data_delta_final(ch0data_delta_final),
  ...
  .A2D_ACLEADOFF_STATP (A2D_ACLEADOFF_STATP), 
   ...
   .leadoff_det_en(leadoff_det_en),
   .leadoff_type_sel_reg(leadoff_type_sel_reg),    //00: N&P all, 01: N only, 10: P only, 11: N&P all
   .switch_duration_sel(switch_duration_sel),    //
   .leadoff_active(leadoff_active),    
   .leadoff_tgt_0(leadoff_tgt_0),
   .leadoff_tgt_1(leadoff_tgt_1),
  ...
  .presetn(presetn),
  //
  .imeas_input_format(imeas_input_format), //from SPI
  .imeas_grp_ctl(imeas_grp_ctl),
  .active_channel_num(active_channel_num),
  .reg_ctrl(imeas_reg_ctrl),//input  wire [15:0] 
  .reg_ch(imeas_reg_ch),//input  wire [6:0]  
  .reg_seq(imeas_reg_seq),//input  wire [2:0]  

  .reg_rstval(imeas_reg_rstval),//input  wire [7:0]  
  .int_clr(imeas_int_clr),//input  wire	   
  .ch0data(imeas_ch0data),//output wire   [15:0]   
  .ch1data(imeas_ch1data),//output wire   [15:0]   
  .ch2data(imeas_ch2data),//output wire   [15:0]   
  //.int_set(imeas_int_set),//output wire   
  ...
  .int_alarm_clr(imeas_int_alarm_clr),
  .imeas_int_alarm_sts(imeas_int_alarm_sts),
  .imeas_int_alarm(imeas_int_alarm), // to io buf
  ...
  .imeas_int_sts(imeas_int_sts), //to SPI
  .imeas_int_sts0(imeas_int_sts0), //to SPI
  .imeas_int_sts1(imeas_int_sts1), //to SPI
  .imeas_bio_int_sts(imeas_bio_int_sts), //to SPI
...
*/
 ...
 .ch0data_delta_final(ch0data_delta_final),
  ...
  .imeas_ch0data(imeas_ch0data),     // Channel 0 data
  .imeas_ch1data(imeas_ch1data),     // Channel 1 data
  .imeas_ch2data(imeas_ch2data),     // Channel 2 data
  .imeas_int_sts(imeas_int_sts),
  .imeas_int_sts0(imeas_int_sts0),
  .imeas_int_sts1(imeas_int_sts1),
  .imeas_bio_int_sts(imeas_bio_int_sts),
  .imeas_int_alarm_sts(imeas_int_alarm_sts),
  .int_sts_switch_loff_statn(int_sts_switch_loff_statn),
  .int_sts_duration_loff_statn(int_sts_duration_loff_statn),
  .int_sts_switch_loff_statp(int_sts_switch_loff_statp),
  ...
  .imeas_reg_ctrl(imeas_reg_ctrl),//input  wire [15:0]
  .imeas_reg_ch(imeas_reg_ch),//input  wire [6:0]
  .imeas_reg_seq(imeas_reg_seq),//input  wire [2:0]
  .imeas_reg_rstval(imeas_reg_rstval),//input  wire [7:0]
  .imeas_int_clr(imeas_int_clr),//input  wire
  .imeas_int_alarm_clr(imeas_int_alarm_clr),//input  wire
