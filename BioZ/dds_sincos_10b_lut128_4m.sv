...
module dds_sincos_10b_lut128_4m #(
    ...
    localparam integer USED_PHASE_W = 2 + LUT_ADDR_W;
    localparam integer DISCARD_W = PHASE_W - USED_PHASE_W;
    localparam signed [DITHER_W:0] DITHER_HALF =
        {1'b0, 1'b1, {(DITHER_W-1){1'b0}}};

    reg [15:0] dither_lfsr;

    wire dither_feedback =
        dither_lfsr[15] ^ dither_lfsr[13] ^ dither_lfsr[12] ^ dither_lfsr[10];

    wire [DITHER_W-1:0] dither_unsigned = dither_lfsr[DITHER_W-1:0];

    wire signed [DITHER_W:0] dither_centered =
        $signed({1'b0, dither_unsigned}) - DITHER_HALF;

    wire signed [PHASE_W-1:0] dither_centered_ext =
        {{(PHASE_W-DITHER_W-1){dither_centered[DITHER_W]}}, dither_centered};

    wire [PHASE_W-1:0] phase_dither =
        DITHER_EN ? (dither_centered_ext <<< (DISCARD_W - DITHER_W)) :
        {PHASE_W{1'b0}};

	
    reg  signed [9:0]       sin_out;
    reg  signed [9:0]       cos_out;
    wire [PHASE_W-1:0] sin_phase = phase_acc[PHASE_W-1:0] +  phase_dither + phase_offset;
    wire [PHASE_W-1:0] cos_phase = phase_acc[PHASE_W-1:0] +  phase_dither + phase_offset_c + PHASE_90;
    ...
    always @(posedge clk or negedge rstn) begin
        if (~rstn) begin
            ...
            sin_out   <= 10'sd0;
            cos_out   <= 10'sd0;
	    dither_lfsr <= (DITHER_SEED == 16'h0000) ? 16'hACE1 : DITHER_SEED;
        end else if (enable) begin
            sin_out   <= sine_from_phase(sin_phase);
            cos_out   <= sine_from_phase(cos_phase);
            phase_acc <= phase_acc + phase_inc;
	    dither_lfsr <= {dither_lfsr[14:0], dither_feedback};
        end else begin
            phase_acc <= {(PHASE_W+1'b1){1'b0}};
            sin_out   <= 10'sd0;
            cos_out   <= 10'sd0;
...
wire   i_square_inv;
assign i_square_inv = ~sin_phase[PHASE_W-1];
    always @(posedge clk or negedge rstn) begin
        if (~rstn)
		i_square <= 1'b0;
	else if(enable) 
		//i_square <=  i_square_inv[PHASE_W-1];
		i_square <=  i_square_inv;
	else 
		i_square <= 1'b0;
...
wire   q_square_inv;
assign q_square_inv = ~cos_phase[PHASE_W-1];
    always @(posedge clk or negedge rstn) begin
        if (~rstn)
		q_square <= 1'b0;
	else if(enable)
		//q_square <= q_square_inv[PHASE_W-1];
		q_square <= q_square_inv;
	else 
		q_square <= 1'b0;
