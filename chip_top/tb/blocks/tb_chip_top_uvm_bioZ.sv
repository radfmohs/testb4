...
nnc_bioZ_interface       bioZ_mon_if();


// Changing clocks
//assign `ANA_TOP.u_zmeas_analog.zmeas_freq_sel = dut_vif.zmeas_freq_sel;
//
//assign zmeas_mon_if.pclk                = `BIOZ_TOP.clk;
//
//assign zmeas_mon_if.mon_bioz_en         = `BIOZ_TOP.enable;
//assign zmeas_mon_if.mon_phase_inc       = `BIOZ_TOP.phase_inc;
//assign zmeas_mon_if.mon_phase_offset    = `BIOZ_TOP.phase_offset;
//assign zmeas_mon_if.mon_dc_data         = `BIOZ_TOP.dc_data;
//assign zmeas_mon_if.mon_dc_data_c       = `BIOZ_TOP.dc_data_c;
//
//assign zmeas_mon_if.mon_data_type_sel   = `BIOZ_TOP.data_type_sel;
//
//assign zmeas_mon_if.mon_sin_unsigned    = `BIOZ_TOP.sin_unsigned;
//assign zmeas_mon_if.mon_cos_unsigned    = `BIOZ_TOP.cos_unsigned;
//assign zmeas_mon_if.mon_i_square        = `BIOZ_TOP.i_square;
//assign zmeas_mon_if.mon_q_square        = `BIOZ_TOP.q_square;
//assign zmeas_mon_if.mon_phase_acc       = `BIOZ_TOP.phase_acc;
//
...
initial begin
    nnc_config_db#(virtual nnc_bioZ_interface)::set(uvm_root::get(), "*", "bioZ_mon_if", bioZ_mon_if);
end
