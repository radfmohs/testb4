...
class `TESTNAME extends soc_base_test;
  ...
  virtual task main_phase(uvm_phase phase);
    ...
    `nnc_info("SOC_TEST", "soc_pinmux_debugmode_atm4_test start", UVM_LOW)
...
`ifndef FPGA
    ...
    force `SOC_TB.IOBUF_PAD[9:6] = 4'b0101;  
    ...
    if (top_test_cfg.atm !== (13'b1_0000_0000_0000 >> `SOC_TB.IOBUF_PAD[9:6]-1))
      begin
        `nnc_error("ATM4",$sformatf("ATM[0:12] = %b is not as expectation of ATM = %b", top_test_cfg.atm, (13'b1_0000_0000_0000 >> `SOC_TB.IOBUF_PAD[9:6]-1))); 
      end

    if (`ANA_TOP.D2A_BIST_EN !== 1'b0)
      `nnc_error("ATM4",$sformatf("[ERROR] ANA_TOP.D2A_BIST_EN = %b is not as expectation 1'b0", `ANA_TOP.D2A_BIST_EN));

    if (`ANA_TOP.D2A_ECGCAL_EN !== 1'b0)
      `nnc_error("ATM4",$sformatf("[ERROR] ANA_TOP.D2A_ECGCAL_EN = %b is not as expectation 1'b0", `ANA_TOP.D2A_ECGCAL_EN));

      
    //Check default signal           
    if (`ANA_TOP.D2A_EN_PPG_BUFFER !== 1'b1)
      begin
        `nnc_error("ATM4", $sformatf("D2A_EN_PPG_BUFFER = %b is not as expectation of 1'b1", `ANA_TOP.D2A_EN_PPG_BUFFER))
      end  
    if (`ANA_TOP.D2A_EN_TIA_VREFBUFFER !== 1'b1)
      begin
        `nnc_error("ATM4", $sformatf("D2A_EN_TIA_VREFBUFFER = %b is not as expectation of 1'b1", `ANA_TOP.D2A_EN_TIA_VREFBUFFER))
      end  
    if (`ANA_TOP.D2A_PPG_TEST_OUT[1:0] !== 2'b00)
      begin
        `nnc_error("ATM4", $sformatf("D2A_PPG_TEST_OUT[1:0] = %b is not as expectation of 2'b0", `ANA_TOP.D2A_PPG_TEST_OUT[1:0]))
      end  
    if (`ANA_TOP.D2A_PDVREF_SEL[1:0] !== 2'b10)
      begin
        `nnc_error("ATM4", $sformatf("D2A_PDVREF_SEL[1:0] = %b is not as expectation of 2'b10", `ANA_TOP.D2A_PDVREF_SEL[1:0]))
      end  
    if (`ANA_TOP.D2A_TIA_IDAC[3:1] !== 3'b000)
      begin
        `nnc_error("ATM4", $sformatf("D2A_TIA_IDAC[3:1] = %b is not as expectation of 3'b000", `ANA_TOP.D2A_TIA_IDAC[3:1]))
      end 
    if (`ANA_TOP.D2A_EN_PPG_SH !== 1'b1)
      begin
        `nnc_error("ATM4", $sformatf("D2A_EN_PPG_SH = %b is not as expectation of 1'b1", `ANA_TOP.D2A_EN_PPG_SH))
      end 
    if (`ANA_TOP.D2A_EN_PPG_AF !== 1'b1)
      begin
        `nnc_error("ATM4", $sformatf("D2A_EN_PPG_AF = %b is not as expectation of 1'b1", `ANA_TOP.D2A_EN_PPG_AF))
      end 
    if (`ANA_TOP.D2A_EN_TIA !== 1'b1)
      begin
        `nnc_error("ATM4", $sformatf("D2A_EN_TIA = %b is not as expectation of 1'b1", `ANA_TOP.D2A_EN_TIA))
      end 

    // Checking pin D2A_SDM_VIN_SEL 
    for (int i=0; i < 100; i++) begin
      force {`SOC_TB.IOBUF_PAD[10],`SOC_TB.IOBUF_PAD[0],`SOC_TB.IOBUF_PAD[2]} = $random;
      #10000;
      rand_num[2:0] = {`SOC_TB.IOBUF_PAD[10],`SOC_TB.IOBUF_PAD[0],`SOC_TB.IOBUF_PAD[2]};     
      if (`ANA_TOP.D2A_SDM_VIN_SEL !== rand_num[2:0]) begin
        `nnc_error("ATM4",$sformatf("[ERROR][%d] D2A_SDM_VIN_SEL = %b is not as expectation of rand_num[2:0] = %b", i, `ANA_TOP.D2A_SDM_VIN_SEL , rand_num[2:0]));
      end
      release {`SOC_TB.IOBUF_PAD[10],`SOC_TB.IOBUF_PAD[0],`SOC_TB.IOBUF_PAD[2]};
      #10000;
    end

    // Checking pin D2A_TIA_GAIN 
    for (int i=0; i < 100; i++) begin
      force {`SOC_TB.IOBUF_PAD[1],`SOC_TB.IOBUF_PAD[3],`SOC_TB.IOBUF_PAD[4],`SOC_TB.IOBUF_PAD[11]} = $random;
      #10000;
      rand_num[3:0] = {`SOC_TB.IOBUF_PAD[1],`SOC_TB.IOBUF_PAD[3],`SOC_TB.IOBUF_PAD[4],`SOC_TB.IOBUF_PAD[11]};     
      if (`ANA_TOP.D2A_TIA_GAIN !== rand_num[3:0]) begin
        `nnc_error("ATM4",$sformatf("[ERROR][%d] D2A_TIA_GAIN `ANA_TOP.D2A_TIA_GAIN = %b is not as expectation of rand_num[3:0] = %b", i, `ANA_TOP.D2A_TIA_GAIN , rand_num[3:0]));
      end
      release {`SOC_TB.IOBUF_PAD[1],`SOC_TB.IOBUF_PAD[3],`SOC_TB.IOBUF_PAD[4],`SOC_TB.IOBUF_PAD[11]};
      #10000;
    end

    // Checking pin D2A_PPG_SH_CK
    for (int i=0; i < 100; i++) begin
      force  `SOC_TB.IOBUF_PAD[5] = $random;
      #10000;
      rand_bit = `SOC_TB.IOBUF_PAD[5];     
      if (`ANA_TOP.D2A_PPG_SH_CK !== rand_bit) begin
        `nnc_error("ATM4",$sformatf("[ERROR][%d] D2A_PPG_SH_CK = %b is not as expectation of rand_bit = %b", i, `ANA_TOP.D2A_PPG_SH_CK , rand_bit));
      end
      release `SOC_TB.IOBUF_PAD[5];
      #10000;
    end

    // Checking pin D2A_TIA_IDAC[0]
    for (int i=0; i < 100; i++) begin
      force `SOC_TB.IOBUF_PAD[12] = $random;
      #10000;
      rand_bit = `SOC_TB.IOBUF_PAD[12];     
      if (`ANA_TOP.D2A_TIA_IDAC[0] !== rand_bit) begin
        `nnc_error("ATM4",$sformatf("[ERROR][%d] D2A_TIA_IDAC[0] = %b is not as expectation of rand_bit = %b", i, `ANA_TOP.D2A_TIA_IDAC[0] , rand_bit));
      end
      release `SOC_TB.IOBUF_PAD[12];
      #10000;
    end

    // Checking pin D2A_TIA_IDAC[7:4]
    for (int i=0; i < 100; i++) begin
      force {`SOC_TB.IOBUF_PAD[14], `SOC_TB.IOBUF_PAD[18] , `SOC_TB.IOBUF_PAD[17] , `SOC_TB.IOBUF_PAD[13]} = $random;
      #50000;
      rand_num[7:4] = {`SOC_TB.IOBUF_PAD[14], `SOC_TB.IOBUF_PAD[18] , `SOC_TB.IOBUF_PAD[17] , `SOC_TB.IOBUF_PAD[13]};     
      if (`ANA_TOP.D2A_TIA_IDAC[7:4] !== rand_num[7:4]) begin
        `nnc_error("ATM4",$sformatf("[ERROR][%d] D2A_TIA_IDAC[7:4] = %b is not as expectation of rand_num[7:4] = %b", i, `ANA_TOP.D2A_TIA_IDAC[7:4] , rand_num[7:4]));
      end
      release {`SOC_TB.IOBUF_PAD[14], `SOC_TB.IOBUF_PAD[18] , `SOC_TB.IOBUF_PAD[17] , `SOC_TB.IOBUF_PAD[13]};
      #10000;
    end

    // Checking pin D2A_PPG_TEST_IN[1:0]
    for (int i=0; i < 100; i++) begin
      force {`SOC_TB.IOBUF_PAD[15] , `SOC_TB.IOBUF_PAD[16]} = $random;
      #10000;
      rand_num[1:0] = {`SOC_TB.IOBUF_PAD[15] , `SOC_TB.IOBUF_PAD[16]};     
      if (`ANA_TOP.D2A_PPG_TEST_IN[1:0] !== rand_num[1:0]) begin
        `nnc_error("ATM4",$sformatf("[ERROR][%d] D2A_PPG_TEST_IN[1:0] = %b is not as expectation of rand_num[1:0] = %b", i, `ANA_TOP.D2A_PPG_TEST_IN[1:0] , rand_num[1:0]));
      end
      release {`SOC_TB.IOBUF_PAD[15] , `SOC_TB.IOBUF_PAD[16]};
      #10000;
    end
