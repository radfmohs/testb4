...
//--------------------------------------------------------------------------------------
// File Name	: soc_spi_fifo_imeas_group_mode_disable_tag_test_clk_config_min.sv                                                   
// Project	: Nanochap ENS3                                  		        
// Description	: Testcase soc_spi_fifo_imeas_group_mode_disable_tag_test_clk_config_min                                             
...
// -------------------------------------------------
`define TESTNAME soc_spi_fifo_imeas_group_mode_disable_tag_test_clk_config_min
`define TESTCFG soc_spi_fifo_imeas_group_mode_disable_tag_test_clk_config_min_cfg
...
class `TESTCFG extends soc_base_test_cfg;
  ...
  function new (string name = "soc_spi_fifo_imeas_group_mode_disable_tag_test_clk_config_min_cfg");
    super.new(name);
  ...
  // spi_sclk_freq
  constraint c_spi_sclk_freq  { soft spi_sclk_freq == 10; }//10Khz SPI min clk - as fifo gets full if spi is very slow

  // iclk_sel
  constraint c_iclk_sel       { soft iclk_sel == 0; }//128Khz imeas adc clk

  // mclk_sel
  constraint c_mclk_sel       { soft mclk_sel == 0; }//256Khz zmeas adc clk
...
class `TESTNAME extends soc_base_test;
  ...
  virtual task main_phase(uvm_phase phase);
    ...
    `nnc_info("SOC_TEST", "soc_spi_fifo_imeas_group_mode_disable_tag_test_clk_config_min start", UVM_LOW)
...
// Set coversion mode to select group mode channel conversion
...
if (rd_data[7:3] !== 5'b01010) begin
    ...
    `nnc_error(get_full_name(), $sformatf("STATUS REGISTER is WRONG = %h", rd_data));
...
if (rd_data !== 8'h00) begin
    ...
    `nnc_error(get_full_name(), $sformatf("FIFO_READ_POINTER is CURRENT VALUE=%h != EXPECTED VALUE=8'h00", rd_data));
end
...
for (int i = 0; i < loop1; i++) begin
 ...
 `nnc_info("SOC_TEST", " received imeas int sts", UVM_LOW)
// `nnc_info("SOC_TEST", "will clear the imeas int sts", UVM_LOW)
// wr_data = 8'h1; // clearing imeas_int status 
// `WR_NORMAL_REG(`SOC_IMEAS_STATUS, wr_data, top_test_cfg.pads);
 ...
 repeat (`SOC_FIFO_SIZE) begin
  ...
  if (rd_data[1:0] === 3'b11) begin
    ...
    `nnc_error(get_full_name(), $sformatf("IMEAS is not in GROUP mode with TAG = %h", rd_data[1:0]));
  end
...
/*
  ...
  for (int i = 0; i < top_test_cfg.no_of_elements; i++) begin
    ...
    `nnc_error(get_full_name(), $sformatf("READ DATA from FIFO Register Element-%d: %h is NOT EQUAL to THE VALUE from IMEAS to FIFO: %h", i, rd_data_imeas2fifo[15:0], rd_data_fifo[i]));
    end
  end */
 end
...
/*  
  ...
  for (int i = 0; i < top_test_cfg.no_of_elements; i++) begin
    ...
    if (rd_data_imeas2fifo[15:0] !== rd_data_fifo[top_test_cfg.no_of_elements - 1 - i]) begin
      //$display("[ERROR][6] READ DATA from FIFO Register Element-%d: %h is NOT EQUAL to THE VALUE from IMEAS to FIFO: %h", i, rd_data_imeas2fifo[15:0], rd_data_fifo[top_test_cfg.no_of_elements - 1 - i]);
      `nnc_error(get_full_name(), $sformatf("READ DATA from FIFO Register Element-%d: %h is NOT EQUAL to THE VALUE from IMEAS to FIFO: %h", i, rd_data_imeas2fifo[15:0], rd_data_fifo[top_test_cfg.no_of_elements - 1 - i]));
    end
  end */
...
//#3000000; // 3ms
    ...
    // --------------------------------------------------------
    //#10000ns;
    `nnc_info("SOC_TEST", "soc_spi_fifo_imeas_group_mode_disable_tag_test_clk_config_min end now", UVM_LOW)
