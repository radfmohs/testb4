...
//--------------------------------------------------------------------------------------
// File Name	: soc_zmeas_ecg_ddssine_test.sv                                                   
// Project	: Nanochap BAF4P1                                  		        
// Description	: Testcase soc_zmeas_ecg_ddssine_test                                             
// Designer	: ophina@nanochap.com                                                                 
// Date		: 06-06-2024                                                                     
// Revision	: 0.1 Initial version created by script                                 
--------------------------------------------------------------------------------------*/
`define TESTNAME soc_zmeas_ecg_ddssine_test
`define TESTCFG soc_zmeas_ecg_ddssine_test_cfg
...
class `TESTCFG extends soc_base_test_cfg;
  ...
  rand logic        shr_man_en;
       logic [3:0]  shr_man_val;
  //------------------------------------------------------------------------------
  //---------------------------- SPI ---------------------------------------------
  rand logic [7:0] mask;
  rand logic [7:0] pads;
  ...
  function new (string name = "soc_zmeas_ecg_ddssine_test_cfg");
    super.new(name);    
  endfunction: new
  ...
  constraint c_freq_val       { soft freq_val inside {[1:4]}; }
  // top_test_cfg.top_test_cfg.dds_wave_sel values
  constraint c_dds_wave_sel   { soft dds_wave_sel inside {[3:3]}; }
  // top_test_cfg.top_test_cfg.sq_data_lval values
  constraint c_sq_data_lval   { soft sq_data_lval <= (sq_data_hval - 32); sq_data_lval inside {[0:1023]}; }
  // top_test_cfg.top_test_cfg.sq_data_hval values
  constraint c_sq_data_hval   { soft sq_data_hval inside {[32:1023]}; }
...
class `TESTNAME extends soc_base_test;
  ...
  virtual task pre_reset_phase(nnc_phase phase);
    ...
    top_test_cfg.shr_man_val = (10 - $floor($log10(int'(top_test_cfg.sq_data_hval) - int'(top_test_cfg.sq_data_lval)) / $log10(2)));
  ...
  virtual task main_phase(uvm_phase phase);
    ...
    `nnc_info("SOC_TEST", "soc_zmeas_ecg_ddssine_test start", UVM_LOW)
   ...
   //Enable ZMEAS
    ...
    ///-------------to read from the SOC_ZMEAS_EN reg-------------------//
	  ...
	  `nnc_error("SOC_TEST", "[ERROR][0] zmeas not enabled!")
       ...
       if(top_test_cfg.dds_wave_sel === 2'b11) begin
	`nnc_info("SOC_TEST", "DDS wave type: ECG CAL SINE", UVM_LOW)
    ...
    //Set ecg cal sine wave high and low amplitudes
	...
	if(top_test_cfg.shr_man_en === 1'b1) begin
	  `nnc_info("SOC_TEST", "MANUAL ECG CAL MODE", UVM_LOW)
	  top_test_cfg.wr_data[0] = {3'b0,top_test_cfg.shr_man_val,top_test_cfg.shr_man_en};
     //Set shift value
     ////------------- to write in to the SOC_IMEAS_REG_CTRL_1 reg--------------//
         `WR_NORMAL_REG(`SOC_IMEAS_REG_CTRL_1, top_test_cfg.wr_data[0], top_test_cfg.pads);
	end
	else
	  `nnc_info("SOC_TEST", "AUTO ECG CAL MODE", UVM_LOW)
       end
