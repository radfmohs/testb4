...
//--------------------------------------------------------------------------------------
// File Name	: soc_pinmux_debugmode_atm11_test.sv                                                   
...
class `TESTNAME extends soc_base_test;
  ...
  virtual task main_phase(uvm_phase phase);
    ...
    `nnc_info("SOC_TEST", "soc_pinmux_debugmode_atm11_test start", UVM_LOW)
...
`ifndef FPGA
    ...
    force `SOC_TB.IOBUF_PAD[9:6] = 4'b1100;  
    ...
    if (top_test_cfg.atm !== (13'b1_0000_0000_0000 >> `SOC_TB.IOBUF_PAD[9:6]-1))
      begin
        `nnc_error("ATM11",$sformatf("ATM[0:12] = %b is not as expectation of ATM = %b", top_test_cfg.atm, (13'b1_0000_0000_0000 >> `SOC_TB.IOBUF_PAD[9:6]-1))); 
      end

    if (`ANA_TOP.D2A_BIST_EN !== 1'b0)
      `nnc_error("ATM11",$sformatf("[ERROR] ANA_TOP.D2A_BIST_EN = %b is not as expectation 1'b0", `ANA_TOP.D2A_BIST_EN));

    if (`ANA_TOP.D2A_ECGCAL_EN !== 1'b0)
      `nnc_error("ATM11",$sformatf("[ERROR] ANA_TOP.D2A_ECGCAL_EN = %b is not as expectation 1'b0", `ANA_TOP.D2A_ECGCAL_EN));
    
    //Check default signal           
    if (`ANA_TOP.D2A_LEDSEL[0] !== 1'b0)
      begin
        `nnc_error("ATM11", $sformatf("D2A_LEDSEL[0] = %b is not as expectation of 1'b1", `ANA_TOP.D2A_LEDSEL[0]))
      end  
    if (`ANA_TOP.D2A_LED_STANDBYEN !== 1'b1)
      begin
        `nnc_error("ATM11", $sformatf("D2A_LED_STANDBYEN = %b is not as expectation of 1'b1", `ANA_TOP.D2A_LED_STANDBYEN))
      end  
    if (`ANA_TOP.D2A_PPGDAC0_EN !== 1'b1)
      begin
        `nnc_error("ATM11", $sformatf("D2A_PPGDAC0_EN = %b is not as expectation of 1'b1", `ANA_TOP.D2A_PPGDAC0_EN))
      end  
    if (`ANA_TOP.D2A_EN_PPGDAC_BUFFER !== 1'b1)
      begin
        `nnc_error("ATM11", $sformatf("D2A_EN_PPGDAC_BUFFER = %b is not as expectation of 1'b1", `ANA_TOP.D2A_EN_PPGDAC_BUFFER))
      end  
    if (`ANA_TOP.D2A_EN_TIA_VREFBUFFER !== 1'b1)
      begin
        `nnc_error("ATM11", $sformatf("D2A_EN_TIA_VREFBUFFER = %b is not as expectation of 1'b1", `ANA_TOP.D2A_EN_TIA_VREFBUFFER))
      end
    ...
    for (int i=0; i < 100; i++) begin
      ...
      #10000;
      D2A_PPGDAC0_VSEL = {`SOC_TB.IOBUF_PAD[18],`SOC_TB.IOBUF_PAD[5],`SOC_TB.IOBUF_PAD[10],`SOC_TB.IOBUF_PAD[0],`SOC_TB.IOBUF_PAD[2], `SOC_TB.IOBUF_PAD[1], `SOC_TB.IOBUF_PAD[3], `SOC_TB.IOBUF_PAD[4], `SOC_TB.IOBUF_PAD[11], `SOC_TB.IOBUF_PAD[12], `SOC_TB.IOBUF_PAD[14], `SOC_TB.IOBUF_PAD[13]} ;         
      rand_num[11:0] = D2A_PPGDAC0_VSEL;  
      if (`ANA_TOP.D2A_PPGDAC0_VSEL !== rand_num[11:0]) begin
        `nnc_error("ATM11",$sformatf("[ERROR][%d] D2A_PPGDAC0_VSEL = %b is not as expectation of rand_num[11:0] = %b", i, `ANA_TOP.D2A_PPGDAC0_VSEL , rand_num[11:0]));
      end
    ...
    for (int i=0; i < 100; i++) begin
      ...
      if (`ANA_TOP.D2A_LED_EN !== rand_bit) begin
        `nnc_error("ATM11",$sformatf("[ERROR][%d] D2A_LED_EN = %b is not as expectation of rand_bit = %b", i, `ANA_TOP.D2A_LED_EN , rand_bit));
      end
    ...
    for (int i=0; i < 100; i++) begin
      ...
      if (`ANA_TOP.D2A_LEDSEL[1] !== rand_bit) begin
        `nnc_error("ATM11",$sformatf("[ERROR][%d] D2A_LEDSEL[1] = %b is not as expectation of rand_bit = %b", i, `ANA_TOP.D2A_LEDSEL[1] , rand_bit));
      end
    ...
    for (int i=0; i < 100; i++) begin
      ...
      if (`ANA_TOP.D2A_LEDDAC_SEL !== rand_bit) begin
        `nnc_error("ATM11",$sformatf("[ERROR][%d] D2A_LEDDAC_SEL = %b is not as expectation of rand_bit = %b", i, `ANA_TOP.D2A_LEDDAC_SEL , rand_bit));
      end
