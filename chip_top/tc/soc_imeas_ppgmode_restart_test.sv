...
//--------------------------------------------------------------------------------------
// File Name	: soc_imeas_ppgmode_restart_test.sv                                                   
// Project	: Nanochap BAF4P1                                  		        
// Description	: Testcase soc_imeas_ppgmode_restart_test                                             
// Designer	: ophina@nanochap.com                                                                 
// Date		: 06-06-2024                                                                     
// Revision	: 0.1 Initial version created by script                                 
--------------------------------------------------------------------------------------*/
`define TESTNAME soc_imeas_ppgmode_restart_test
`define TESTCFG soc_imeas_ppgmode_restart_test_cfg
...
class `TESTCFG extends soc_base_test_cfg;
  ...
  function new (string name = "soc_imeas_ppgmode_restart_test_cfg");
    super.new(name);    
  endfunction: new
  ...
  constraint c_restart_en     { soft restart_en == 1'b1; }//restart enabled
...
class `TESTNAME extends soc_base_test;
  ...
  virtual task main_phase(uvm_phase phase);
    ...
    `nnc_info("SOC_TEST", "soc_imeas_ppgmode_restart_test start", UVM_LOW)

    // ----------------------------------------------------------------------------------
    // Please add your code of your test here
    // ----------------------------------------------------------------------------------  
    `nnc_info("SOC_TEST",$sformatf("Simulation Begin!!!"),UVM_LOW);
    `nnc_info("SOC_TEST",$sformatf("Imeas single channel ppg mode test with restart enabled"),UVM_LOW);
    do_run;
  ...
  virtual task do_run;
   ...
   //Enable IMEAS
    ...
    ///-------------to read from the SOC_DEVICE_INT_STATUS_0 reg-------------------//
       `RD_NORMAL_REG(`SOC_DEVICE_INT_STATUS_0,top_test_cfg.pads,top_test_cfg.int_sts);
       
    //Check int status
       if((`SOC_TB.imeas_filter0_en === 1) && (top_test_cfg.int_sts[2] !== 1'b1)) begin
	  ...
	  ///-------------to read burst starting from the SOC_IMEAS_CH0DATA_0 reg-------------------//
       ...
       if((`SOC_TB.imeas_filter1_en === 1) && (top_test_cfg.int_sts[3] !== 1'b1)) begin
