...
//--------------------------------------------------------------------------------------
// File Name	: soc_zmeas_restartcalc_test.sv                                                   
// Project	: Nanochap BAF4P1                                  		        
// Description	: Testcase soc_zmeas_restartcalc_test                                             
// Designer	: ophina@nanochap.com                                                                 
// Date		: 06-06-2024                                                                     
// Revision	: 0.1 Initial version created by script                                 
--------------------------------------------------------------------------------------*/
`define TESTNAME soc_zmeas_restartcalc_test
`define TESTCFG soc_zmeas_restartcalc_test_cfg
...
class `TESTCFG extends soc_base_test_cfg;
  ...
  function new (string name = "soc_zmeas_restartcalc_test_cfg");
    super.new(name);    
  endfunction: new
  ...
  //constraint c_repeat_cal     { soft repeat_cal == 1'b0; }//no repeat
...
class `TESTNAME extends soc_base_test;
  ...
  virtual task main_phase(uvm_phase phase);
    ...
    `nnc_info("SOC_TEST", "soc_zmeas_restartcalc_test start", UVM_LOW)

    // ----------------------------------------------------------------------------------
    // Please add your code of your test here
    // ----------------------------------------------------------------------------------  
    `nnc_info("SOC_TEST",$sformatf("Simulation Begin!!!"),UVM_LOW);
    `nnc_info("SOC_TEST",$sformatf("ZMEAS restart calculation by sending NOOP while a calculation is in progress"),UVM_LOW);
    do_run;
   ...
   //Enable ZMEAS
    ...
    ///-------------to read from the SOC_ZMEAS_EN reg-------------------//
	  ...
	  `nnc_error("SOC_TEST", "[ERROR][0] zmeas not enabled!")
       ...
       #1000;
       top_test_cfg.mode = `ZMEAS_POWER_DOWN;
       top_test_cfg.wr_data[0] = {top_test_cfg.mode,5'b0};
       `nnc_info("SOC_TEST", "power-down mode", UVM_LOW)
    ...
    ////------------- to write in to the SOC_ZMEAS_REG_CTRL_1 reg--------------//
       ...
       top_test_cfg.freq_val = `ZMEAS_REG_FREQ_VAL_1KHZ;
       top_test_cfg.repeat_cal = 1'b1;
    ...
    ///-------------to read from the SOC_ZMEAS_REG_STATUS_0 reg-------------------//
       ...
       while(top_test_cfg.status[4] === 0);
       `nnc_info("SOC_TEST", "zmeas adc enabled!", UVM_LOW)
    ...
    ///-------------to read from the SOC_ZMEAS_REG_STATUS_1 reg-------------------//
       ...
       if(top_test_cfg.status[4:3] !== 2'b11)//adc_en,dds_en
         `nnc_error("SOC_TEST", "[ERROR][6] zmeas_status[4:3] is not expected!")
       if(top_test_cfg.status[2:0] !== 0) //measurement in progress
         `nnc_error("SOC_TEST", "[ERROR][7] zmeas_status[2:0] is not expected!")

       top_test_cfg.mode = `ZMEAS_NOOP;
       top_test_cfg.wr_data[0] = {top_test_cfg.mode,top_test_cfg.repeat_cal,1'b0,top_test_cfg.int_en,2'b0};
       `nnc_info("SOC_TEST", "Send NO-OP!", UVM_LOW)
    ////------------- to write in to the SOC_ZMEAS_REG_CTRL_1 reg--------------//
       `WR_NORMAL_REG(`SOC_ZMEAS_REG_CTRL_1, top_test_cfg.wr_data[0], top_test_cfg.pads);

       #10000000;
   //Make sure calculation stopped & interrupt not occured
    ...
    //Check int status
	  ...
	  `nnc_error("SOC_TEST", "[ERROR][8] zmeas stop measurement failed!")
       else
	  `nnc_info("SOC_TEST", "zmeas measurement stopped!", UVM_LOW)
    ...
    ///-------------to read from the SOC_ZMEAS_REG_STATUS_1 reg-------------------//
       ...
       `nnc_info("SOC_TEST", $sformatf("zmeas status is: (0x%x)",top_test_cfg.status),UVM_LOW)
       if(top_test_cfg.status !== 0)
         `nnc_error("SOC_TEST", "[ERROR][9] zmeas_status[15:0] is not expected!")
    ...
    ///-------------to read burst starting from the SOC_ZMEAS_REG_DATAOUT_0 reg-------------------//
       ...
       if(top_test_cfg.zmeas_data !== 32'h0)
         `nnc_error("SOC_TEST", "[ERROR][10] ZMEAS DATA INCORRECT!")

       top_test_cfg.mode = `ZMEAS_INIT;
       top_test_cfg.freq_val = `ZMEAS_REG_FREQ_VAL_2KHZ;
       top_test_cfg.repeat_cal = 1'b0;
       top_test_cfg.wr_data[0] = {top_test_cfg.tx_gain_sel[1],top_test_cfg.calib,top_test_cfg.zmeas_analog_en,top_test_cfg.tx_gain_sel[0],1'b0,top_test_cfg.rx_pga_gain,2'b0};
       top_test_cfg.wr_data[1] = {top_test_cfg.mode,top_test_cfg.repeat_cal,4'b0};
       top_test_cfg.wr_data[2] = {top_test_cfg.settling_time[3:0],1'b0,top_test_cfg.freq_val};
       top_test_cfg.wr_data[3] = {top_test_cfg.repeat_num,top_test_cfg.settling_time[7:4]};
    ...
    //Check int status
	  ...
	  `nnc_error("SOC_TEST", "[ERROR][11] zmeas interrupt failed!")
       else
    ...
    ///-------------to read from the SOC_ZMEAS_REG_STATUS_1 reg-------------------//
       ...
       `nnc_info("SOC_TEST", $sformatf("zmeas status is: (0x%x)",top_test_cfg.status),UVM_LOW)
       if(top_test_cfg.status[15:12] !== top_test_cfg.repeat_num)
         `nnc_error("SOC_TEST", "[ERROR][12] zmeas_status[15:12] is not expected!")
       if(top_test_cfg.status[11:9] !== top_test_cfg.freq_val)
         `nnc_error("SOC_TEST", "[ERROR][13] zmeas_status[11:9] is not expected!")
       if((top_test_cfg.status[8:7] !== {top_test_cfg.zmeas_analog_en,top_test_cfg.tx_gain_sel[0]}) || (top_test_cfg.status[8:7] !== {`ANA_TOP.D2A_SW_Z_EN,`ANA_TOP.D2A_SW_Z_TX_GSEL[0]}))
         `nnc_error("SOC_TEST", "[ERROR][14] zmeas_status[8:7] is not expected!")
       if((top_test_cfg.status[6] !== top_test_cfg.calib) || (top_test_cfg.status[6] !== `ANA_TOP.D2A_SW_Z_CALMD))
         `nnc_error("SOC_TEST", "[ERROR][15] zmeas_status[6] is not expected!")
       if((top_test_cfg.status[5] !== top_test_cfg.rx_pga_gain) || (top_test_cfg.status[5] !== `ANA_TOP.D2A_SW_Z_RXGAIN))
         `nnc_error("SOC_TEST", "[ERROR][16] zmeas_status[5] is not expected!")
       if(top_test_cfg.status[4:3] !== 2'b0)//adc_en,dds_en
         `nnc_error("SOC_TEST", "[ERROR][17] zmeas_status[4:3] is not expected!")
       if(top_test_cfg.status[2:0] !== 7) //measurement is done
         `nnc_error("SOC_TEST", "[ERROR][18] zmeas_status[2:0] is not expected!")
    ...
    ///-------------to read burst starting from the SOC_ZMEAS_REG_DATAOUT_0 reg-------------------//
       ...
       if(top_test_cfg.zmeas_data !== 32'hfbb30670)
         `nnc_error("SOC_TEST", "[ERROR][19] ZMEAS DATA INCORRECT!")
    ...
    //Check int status
       if(top_test_cfg.int_sts[0] !== 1'b0)
           `nnc_error("SOC_TEST", "[ERROR][20] zmeas interrupt not cleared!")
       else
