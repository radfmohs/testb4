...
class `TESTNAME extends soc_base_test;
  ...
  virtual task main_phase(uvm_phase phase);
    ...
    `nnc_info("SOC_TEST", "soc_pinmux_debugmode_atm0_test start", UVM_LOW)
...
`ifndef FPGA
    ...
    force `SOC_TB.IOBUF_PAD[9:6] = 4'b0001;  
    ...
    if (top_test_cfg.atm !== (13'b1_0000_0000_0000 >> `SOC_TB.IOBUF_PAD[9:6]-1))
      begin
        `nnc_error("ATM0",$sformatf("ATM[0:12] = %b is not as expectation of ATM = %b", top_test_cfg.atm, (13'b1_0000_0000_0000 >> `SOC_TB.IOBUF_PAD[9:6]-1))); 
      end

    if (`ANA_TOP.D2A_BIST_EN !== 1'b0)
      `nnc_error("ATM0",$sformatf("[ERROR] ANA_TOP.D2A_BIST_EN = %b is not as expectation 1'b0", `ANA_TOP.D2A_BIST_EN));

    if (`ANA_TOP.D2A_ECGCAL_EN !== 1'b0)
      `nnc_error("ATM0",$sformatf("[ERROR] ANA_TOP.D2A_ECGCAL_EN = %b is not as expectation 1'b0", `ANA_TOP.D2A_ECGCAL_EN));

    // Checking pin D2A_DCLEADOFF_COMP_TH_TRIM[2:0] 
    for (int i=0; i < 100; i++) begin
        ...
        if (`ANA_TOP.D2A_DCLEADOFF_COMP_TH_TRIM[2:0] !== rand_num[2:0]) begin
            `nnc_error("ATM0",$sformatf("[ERROR][%d] D2A_DCLEADOFF_COMP_TH_TRIM[2:0] = %b is not as expectation of rand_num[2:0] = %b", i, `ANA_TOP.D2A_DCLEADOFF_COMP_TH_TRIM[2:0] , rand_num[2:0]));
        end
        release {`SOC_TB.IOBUF_PAD[10],`SOC_TB.IOBUF_PAD[0],`SOC_TB.IOBUF_PAD[2]};
        #10000;
    end
    // Checking pin D2A_DCLEADOFF_ISEL_TRIM[2:0] 
    for (int i=0; i < 100; i++) begin
      force {`SOC_TB.IOBUF_PAD[15],`SOC_TB.IOBUF_PAD[1],`SOC_TB.IOBUF_PAD[3]} = $random;
      #10000;
      rand_num[2:0] = {`SOC_TB.IOBUF_PAD[15],`SOC_TB.IOBUF_PAD[1],`SOC_TB.IOBUF_PAD[3]};     
      if (`ANA_TOP.D2A_DCLEADOFF_ISEL_TRIM[2:0] !== rand_num[2:0]) begin
        `nnc_error("ATM0",$sformatf("[ERROR][%d] D2A_DCLEADOFF_ISEL_TRIM[2:0] = %b is not as expectation of rand_num[2:0] = %b", i, `ANA_TOP.D2A_DCLEADOFF_ISEL_TRIM[2:0], rand_num[2:0]));
      end
      release {`SOC_TB.IOBUF_PAD[15],`SOC_TB.IOBUF_PAD[1],`SOC_TB.IOBUF_PAD[3]};
      #10000;
    end
    ...
    for (int i=0; i < 100; i++) begin
      force `SOC_TB.IOBUF_PAD[4] = $random;
      #50000;
      rand_bit = `SOC_TB.IOBUF_PAD[4];     
      if (`ANA_TOP.D2A_DCLEADOFF_VINFLIP !== rand_bit) begin
        `nnc_error("ATM0",$sformatf("[ERROR][%d] D2A_DCLEADOFF_VINFLIP = %b is not as expectation of rand_bit = %b", i, `ANA_TOP.D2A_DCLEADOFF_VINFLIP , rand_bit));
      end
      release `SOC_TB.IOBUF_PAD[4];
      #10000;
    end

    // Checking pin D2A_DCLEADOFF_EN 
    for (int i=0; i < 100; i++) begin
      force `SOC_TB.IOBUF_PAD[13] = $random;
      #10000;
      rand_bit = `SOC_TB.IOBUF_PAD[13];     
      if (`ANA_TOP.D2A_DCLEADOFF_EN !== rand_bit) begin
        `nnc_error("ATM0",$sformatf("[ERROR][%d] D2A_DCLEADOFF_EN = %b is not as expectation of rand_bit = %b", i, `ANA_TOP.D2A_DCLEADOFF_EN , rand_bit));
      end
      release `SOC_TB.IOBUF_PAD[13];
      #10000;
    end

    // Checking pin D2A_DCLEADOFF_RMODE_EN 
    for (int i=0; i < 100; i++) begin
      force `SOC_TB.IOBUF_PAD[11] = $random;
      #10000;
      rand_bit = `SOC_TB.IOBUF_PAD[11];     
      if (`ANA_TOP.D2A_DCLEADOFF_RMODE_EN !== rand_bit) begin
        `nnc_error("ATM0",$sformatf("[ERROR][%d] D2A_DCLEADOFF_RMODE_EN = %b is not as expectation of rand_bit = %b", i, `ANA_TOP.D2A_DCLEADOFF_RMODE_EN , rand_bit));
      end
      release `SOC_TB.IOBUF_PAD[11];
      #10000;
    end

    // Checking pin A2D_LOFF_STATN 
    for (int i=0; i < 100; i++) begin
      force `ANA_TOP.A2D_LOFF_STATN = $random;
      #10000;
      rand_bit = `ANA_TOP.A2D_LOFF_STATN;     
      if (`SOC_TB.IOBUF_PAD[12] !== rand_bit) begin
        `nnc_error("ATM0",$sformatf("[ERROR][%d] A2D_LOFF_STATN = %b is not as expectation of rand_bit = %b", i, `ANA_TOP.A2D_LOFF_STATN , rand_bit));
      end
      release `ANA_TOP.A2D_LOFF_STATN;
      #10000;
    end

    // Checking pin A2D_LOFF_STATP 
    for (int i=0; i < 100; i++) begin
      force `ANA_TOP.A2D_LOFF_STATP = $random;
      #10000;
      rand_bit = `ANA_TOP.A2D_LOFF_STATP;     
      if (`SOC_TB.IOBUF_PAD[14] !== rand_bit) begin
        `nnc_error("ATM0",$sformatf("[ERROR][%d] A2D_LOFF_STATP = %b is not as expectation of rand_bit = %b", i, `ANA_TOP.A2D_LOFF_STATP , rand_bit));
      end
...
`ifdef POSTLAYOUT
    if(`SOC_TOP.IOBUF_PU_always_on[2:0] !== 3'h0)
      begin 
        `nnc_error("ATM0",$sformatf("The value of `SOC_TOP.IOBUF_PU[2:0] = %b is not equal to expected value = %b", `SOC_TOP.IOBUF_PU_always_on[2:0], 3'h0));
      end
`elsif POSTSCAN
    if(`SOC_TOP.IOBUF_PU[2:0] !== 3'h7)
      begin 
        `nnc_error("ATM0",$sformatf("The value of `SOC_TOP.IOBUF_PU_always_on[2:0] = %b is not equal to expected value = %b", `SOC_TOP.IOBUF_PU[2:0], 3'h7));
      end
`else
