...
//--------------------------------------------------------------------------------------
// File Name	: soc_zmeas_norepeat_freq2k_test.sv                                                   
// Project	: Nanochap BAF4P1                                  		        
// Description	: Testcase soc_zmeas_norepeat_freq2k_test                                             
// Designer	: ophina@nanochap.com                                                                 
// Date		: 06-06-2024                                                                     
// Revision	: 0.1 Initial version created by script                                 
--------------------------------------------------------------------------------------*/
`define TESTNAME soc_zmeas_norepeat_freq2k_test
`define TESTCFG soc_zmeas_norepeat_freq2k_test_cfg
...
class `TESTCFG extends soc_base_test_cfg;
  ...
  function new (string name = "soc_zmeas_norepeat_freq2k_test_cfg");
    super.new(name);    
  endfunction: new
  ...
  constraint c_int_en         { soft int_en == 1'b0; }//interrupt disabled
...
class `TESTNAME extends soc_base_test;
  ...
  virtual task main_phase(uvm_phase phase);
    ...
    `nnc_info("SOC_TEST", "soc_zmeas_norepeat_freq2k_test start", UVM_LOW)

    // ----------------------------------------------------------------------------------
    // Please add your code of your test here
    // ----------------------------------------------------------------------------------  
    `nnc_info("SOC_TEST",$sformatf("Simulation Begin!!!"),UVM_LOW);
    `nnc_info("SOC_TEST",$sformatf("ZMEAS test without repeat(freq_val:2KHz)"),UVM_LOW);
    do_run;
  ...
  virtual task do_run;
   ...
   //Enable ZMEAS
    ...
    ////------------- to write in to the SOC_ZMEAS_REG_CTRL_1 reg--------------//
       ...
       `nnc_info("SOC_TEST", "Disable zmeas interrupt", UVM_LOW)
     ////------------- to write in to the SOC_ZMEAS_REG_CTRL_1 reg--------------//
       `WR_NORMAL_REG(`SOC_ZMEAS_REG_CTRL_1, top_test_cfg.wr_data[0], top_test_cfg.pads);

       //Wait for zmeas interrupt status
    ///-------------to read from the SOC_ZMEAS_INT reg-------------------//
       do
         `RD_NORMAL_REG(`SOC_ZMEAS_INT,top_test_cfg.pads,top_test_cfg.int_sts);
       while(top_test_cfg.int_sts[0] === 0);
       `nnc_info("SOC_TEST", "zmeas calculation completed!", UVM_LOW)

    //Check zmeas interrupt
       if(`SOC_TB.IOBUF_PAD[4] === 1'b0)
	  `nnc_info("SOC_TEST", "zmeas interrupt not occurred!", UVM_LOW)
       else
	  `nnc_error("SOC_TEST", "[ERROR][1] zmeas interrupt disable failed!")
