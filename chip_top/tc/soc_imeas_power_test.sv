...
//--------------------------------------------------------------------------------------
// File Name	: soc_imeas_power_test.sv                                                   
// Project	: Nanochap BAF4P1                                  		        
// Description	: Testcase soc_imeas_power_test                                             
// Designer	: ophina@nanochap.com                                                                 
// Date		: 06-06-2024                                                                     
// Revision	: 0.1 Initial version created by script                                 
--------------------------------------------------------------------------------------*/
`define TESTNAME soc_imeas_power_test
`define TESTCFG soc_imeas_power_test_cfg
...
class `TESTCFG extends soc_base_test_cfg;
  ...
  function new (string name = "soc_imeas_power_test_cfg");
    super.new(name);    
  endfunction: new
...
class `TESTNAME extends soc_base_test;
  ...
  virtual task main_phase(uvm_phase phase);
    ...
    `nnc_info("SOC_TEST", "soc_imeas_power_test start", UVM_LOW)

    // ----------------------------------------------------------------------------------
    // Please add your code of your test here
    // ----------------------------------------------------------------------------------  
    `nnc_info("SOC_TEST",$sformatf("Simulation Begin!!!"),UVM_LOW);
    `nnc_info("SOC_TEST",$sformatf("Imeas single channel test"),UVM_LOW);
    do_run;
  ...
  virtual task do_run;
    begin
       ...
       top_test_cfg.wr_data[0] = 8'hF9;
   //Dsable bio, ppg, fifo, flash
    ////------------- to write in to the SOC_PMU_REG0 reg--------------//
        `WR_NORMAL_REG(`SOC_PMU_REG0, top_test_cfg.wr_data[0], top_test_cfg.pads);
