...
//--------------------------------------------------------------------------------------
// File Name	: soc_imeas_bio_electricity_filter_test_extended.sv                                                   
// Project	: Nanochap BAF4P1                                  		        
// Description	: Testcase soc_imeas_bio_electricity_filter_test_extended                                             
// Designer	: shreeyal@nanochap.com                                                                 
// Date		: 02-08-2024                                                                     
// Revision	: 0.1 Initial version created by script                                 
--------------------------------------------------------------------------------------*/
`define TESTNAME soc_imeas_bio_electricity_filter_test_extended
`define TESTCFG soc_imeas_bio_electricity_filter_test_extended_cfg
...
class `TESTCFG extends soc_imeas_bio_electricity_filter_test_cfg;
  ...
  function new (string name = "soc_imeas_bio_electricity_filter_test_extended_cfg");
    super.new(name);    
  endfunction: new
  ...
  //constraint c_imeas_sin_expected_freq  { imeas_sin_expected_freq inside {[1:1000]}; imeas_sin_expected_freq != 49; imeas_sin_expected_freq != 50; imeas_sin_expected_freq != 51; }
  ...
  `ifdef POSTLAYOUT_PG 
  constraint c_imeas_rtl_bypass_en    { imeas_rtl_bypass_en == 1'b0; } //Run with IMEAS only in case of postlayout as running without IMEAS is not relaible beacause of clk en
  `elsif POSTSCAN_PG
  constraint c_imeas_rtl_bypass_en    { imeas_rtl_bypass_en == 1'b0; } //Run with IMEAS only in case of postlayout as running without IMEAS is not relaible beacause of clk en
  `endif
...
class `TESTNAME extends soc_imeas_bio_electricity_filter_test;
  ...
  virtual task pre_reset_phase(nnc_phase phase);
    ...
    assert(top_test_cfg.randomize() with { imeas_sin_expected_freq inside {[1:1000]};imeas_sin_expected_freq != 49; imeas_sin_expected_freq != 50; imeas_sin_expected_freq != 51; imeas_sin_expected_freq < 256000/(5 * 2**(iclk_sel+1+cic_rate+5)); });
    ...
    `FILTER_POINT_TO_POINT_SB_EN = 1;
    `FILTER_PEAK_TO_PEAK_SB_EN = 0;

    phase.drop_objection(this);
  endtask : pre_reset_phase

  virtual task main_phase(uvm_phase phase);
    `nnc_info(get_full_name(), $sformatf("imeas_rtl_bypass_en = %0d ",`DUT_IF.imeas_rtl_bypass_en),UVM_LOW);
