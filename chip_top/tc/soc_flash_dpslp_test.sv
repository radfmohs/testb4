...
//--------------------------------------------------------------------------------------
// File Name	: soc_flash_dpslp_test.sv                                                   
// Project	: Nanochap ENS1P4                                  		        
// Description	: Testcase soc_flash_dpslp_test                                             
// Designer	: ophina@nanochap.com                                                                 
// Date		: 29-11-1623                                                                     
// Revision	: 0.1 Initial version created by script                                 
--------------------------------------------------------------------------------------*/
`define TESTNAME soc_flash_dpslp_test
`define TESTCFG soc_flash_dpslp_test_cfg
...
class `TESTCFG extends soc_base_test_cfg;
  ...
  function new (string name = "soc_flash_dpslp_test_cfg");
    super.new(name);    
  endfunction: new
...
class `TESTNAME extends soc_base_test;
  ...
  virtual task main_phase(uvm_phase phase);
    ...
    `nnc_info("SOC_TEST", "soc_flash_dpslp_test start", UVM_LOW)
  ...
  virtual task do_run;
	...
	`nnc_info("SOC_TEST",$sformatf("\tana trim data_%d: (0x%x)\n", i, top_test_cfg.ana_trim_data[i]),UVM_LOW);
        ...
        `nnc_info("SOC_TEST",$sformatf("\n Enable fclk dynamic_en"),UVM_LOW);
        //Set fclk_dyn_en
        ////------------- to write in to the SOC_CLK_CTRL_REG reg--------------//
        `WR_NORMAL_REG(`SOC_CLK_CTRL_REG,8'h01,top_test_cfg.pad);
        `DUT_IF.pclk_sel = 2'h0;

        `nnc_info("SOC_TEST",$sformatf("\n Enter DEEPSLEEP mode"),UVM_LOW);
        //Set FLASH DEEPSLEEP
        ////------------- to write in to the SOC_PMU_REG0 reg--------------//
        `WR_NORMAL_REG(`SOC_PMU_REG0,8'h0B,top_test_cfg.pad);

        //wait (!`SPI_TOP.FLASH_DEEPPD);
        `RD_NORMAL_REG(`SOC_FLASH_DEBUG2,top_test_cfg.pad,top_test_cfg.flash_debug_status);
        while (top_test_cfg.flash_debug_status[4]===0) begin
	    `RD_NORMAL_REG(`SOC_FLASH_DEBUG2,top_test_cfg.pad,top_test_cfg.flash_debug_status);
        end
        `nnc_info("SOC_TEST",$sformatf("\tFlash enters Deep Power Down mode"),UVM_LOW);

        if(`FLASH_TOP.u_32k.DEEPPD != 1) begin
            `nnc_error("SOC_TEST",$sformatf("\tERROR!!! DEEPPD not set!!!"));
        end

        #100000;
        `nnc_info("SOC_TEST",$sformatf("\n Send WAKEUP Reset Request"),UVM_LOW);
        //Set RESETREQ
        ////------------- to write in to the SOC_PMU_REG0 reg--------------//
        `WR_NORMAL_REG(`SOC_PMU_REG0,8'h0F,top_test_cfg.pad);

        //wait (!`SPI_TOP.FLASH_DEEPPD);
        `RD_NORMAL_REG(`SOC_FLASH_DEBUG2,top_test_cfg.pad,top_test_cfg.flash_debug_status);
        while (top_test_cfg.flash_debug_status[4]===1) begin
	    `RD_NORMAL_REG(`SOC_FLASH_DEBUG2,top_test_cfg.pad,top_test_cfg.flash_debug_status);
        end
        `nnc_info("SOC_TEST",$sformatf("\tFlash enters Normal mode"),UVM_LOW);
        if(`FLASH_TOP.u_32k.DEEPPD != 0) begin
            `nnc_error("SOC_TEST",$sformatf("\tERROR!!! DEEPPD not cleared!!!"));
        end

        #10000;
        `nnc_info("SOC_TEST",$sformatf("\n Disable fclk dynamic_en"),UVM_LOW);
        //CLear fclk_dyn_en
        ////------------- to write in to the SOC_CLK_CTRL_REG reg--------------//
        `WR_NORMAL_REG(`SOC_CLK_CTRL_REG,8'h00,top_test_cfg.pad);
        `DUT_IF.pclk_sel = 2'h0;
