...
//--------------------------------------------------------------------------------------
// File Name	: soc_nirs_ppg_receiver_master_cont_typ_mode_corner_case_test.sv                                                   
// Project	: Nanochap ENS2                                  		        
// Description	: Testcase soc_nirs_ppg_receiver_master_cont_typ_mode_corner_case_test                                             
...
// -------------------------------------------------
`define TESTNAME soc_nirs_ppg_receiver_master_cont_typ_mode_corner_case_test
`define TESTCFG soc_nirs_ppg_receiver_master_cont_typ_mode_corner_case_test_cfg
...
class `TESTCFG extends soc_nirs_ppg_base_test_cfg;
  ...
  function new (string name = "soc_nirs_ppg_receiver_master_cont_typ_mode_corner_case_test_cfg");
    super.new(name);
  ...
  constraint c_spi_sclk_freq          {  spi_sclk_freq == 16000;} //16MHZ TYPICAL (As Phil confirmed application migth use typical spi clock as 16MHZ)
  ...
  //
  constraint c_rd_gen_int_sts_or_nirs_dout_chx {rd_gen_int_sts_or_nirs_dout_chx ==1'b1;}
  //  
...
class `TESTNAME extends soc_nirs_ppg_base_test;
  ...
  virtual task main_phase(nnc_phase phase);
    ...
    `nnc_info("SOC_TEST", "soc_nirs_ppg_receiver_master_cont_typ_mode_corner_case_test start", NNC_LOW)
    ...
    for(top_test_cfg.num_leds =0; top_test_cfg.num_leds < top_test_cfg.num_of_leds_loop; top_test_cfg.num_leds++ )begin  //led0 led1
        ...
        for (top_test_cfg.ch = 0; top_test_cfg.ch < 1; top_test_cfg.ch++) begin  //ch0 to ch7
          ...
          assert(`NIRS_PPG_CTRL_CFG.randomize() with{nirs_ppg_mode_sel == 'h0;
                                                     nirs_ppg_led_ambient_en == 1'b0;//((ctrl_ambient_mode == 1'b1) ? 1'b1 : 1'b0);
                                                     ...
                                                     nirs_ppg_led_signle_en == 1'b1;//((ctrl_single_dual_led_mode == 1'b1)? 1'b1 : 1'b0); //0(dual led), 1(single led) //nirs_ppg_led_signle_en == 'h0;
                                                     idac_min_int_en == 'h0;
                                                     idac_max_int_en == 'h0;
                                                     ...
                                                     ana_ppg_clk_div == 2'b00;
                                                     on_time_sel == 4'b0;
                                                     reset_on_time_ctrl == 3'b0;
...
/*   `nnc_info("PPG_TEST",$sformatf("top_test_cfg.ch_en_mask=%0h",top_test_cfg.ch_en_mask),NNC_LOW); 
   `nnc_info("PPG_TEST",$sformatf("define num_of_leds in loop =%0h",top_test_cfg.num_of_leds_loop ),NNC_LOW); 
    ...
    // --------------------------------------------------------
    #10000ns;
    `nnc_info("SOC_TEST", "soc_nirs_ppg_receiver_master_cont_typ_mode_corner_case_test end now", NNC_LOW)
         ...
         begin
           `nnc_info("PPG_TEST","RE-Randomize IREFCOARSE and IREF FINE \n",NNC_LOW);
            ...
            for(int i=0; i<8; i++)begin
               `DUT_IF.nirs_irefcoarse_length[i] = 32'd125; //top_test_cfg.nirs_irefcoarse_length[0];
               `DUT_IF.nirs_irefcoarse_iref_delay[i] = 32'd125; //top_test_cfg.nirs_irefcoarse_iref_delay[0];
               `DUT_IF.nirs_ireffine_length[i] = 32'd125; //top_test_cfg.nirs_ireffine_length[0];
           end
           fork
              begin
                fork
