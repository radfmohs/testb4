...
//--------------------------------------------------------------------------------------
// File Name	: soc_pinmux_debugmode_atm9_test.sv                                                   
...
class `TESTNAME extends soc_base_test;
  ...
  virtual task main_phase(uvm_phase phase);
    ...
    `nnc_info("SOC_TEST", "soc_pinmux_debugmode_atm9_test start", UVM_LOW)
...
`ifndef FPGA
    ...
    force `SOC_TB.IOBUF_PAD[9:6] = 4'b1010;  
    ...
    if (top_test_cfg.atm !== (13'b1_0000_0000_0000 >> `SOC_TB.IOBUF_PAD[9:6]-1))
      begin
        `nnc_error("ATM9",$sformatf("ATM[0:12] = %b is not as expectation of ATM = %b", top_test_cfg.atm, (13'b1_0000_0000_0000 >> `SOC_TB.IOBUF_PAD[9:6]-1))); 
      end

    if (`ANA_TOP.D2A_BIST_EN !== 1'b0)
      `nnc_error("ATM9",$sformatf("[ERROR] ANA_TOP.D2A_BIST_EN = %b is not as expectation 1'b0", `ANA_TOP.D2A_BIST_EN));

    if (`ANA_TOP.D2A_ECGCAL_EN !== 1'b0)
      `nnc_error("ATM9",$sformatf("[ERROR] ANA_TOP.D2A_ECGCAL_EN = %b is not as expectation 1'b0", `ANA_TOP.D2A_ECGCAL_EN));

    // Checking pin D2A_RLD_CHOP_EN 
    for (int i=0; i < 100; i++) begin
      force `SOC_TB.IOBUF_PAD[0] = $random;
      #50000;
      rand_bit = `SOC_TB.IOBUF_PAD[0];  
      if (`ANA_TOP.D2A_RLD_CHOP_EN !== rand_bit) begin
        `nnc_error("ATM9",$sformatf("[ERROR][%d] D2A_RLD_CHOP_EN = %b is not as expectation of rand_bit = %b", i, `ANA_TOP.D2A_RLD_CHOP_EN , rand_bit));
      end
    ...
    // Checking pin D2A_2ELECTRODE_EN 
    for (int i=0; i < 100; i++) begin
      force `SOC_TB.IOBUF_PAD[15] = $random;
      #10000;
      rand_bit = `SOC_TB.IOBUF_PAD[15];  
      if (`ANA_TOP.D2A_2ELECTRODE_EN !== rand_bit) begin
        `nnc_error("ATM9",$sformatf("[ERROR][%d] D2A_2ELECTRODE_EN = %b is not as expectation of rand_bit = %b", i, `ANA_TOP.D2A_2ELECTRODE_EN , rand_bit));
      end
      release `SOC_TB.IOBUF_PAD[15];
      #10000;
    end

    // Checking pin D2A_RLD_CLK64K 
    for (int i=0; i < 100; i++) begin
      force `SOC_TB.IOBUF_PAD[16] = $random;
      #10000;
      rand_bit = `SOC_TB.IOBUF_PAD[16];  
      if (`ANA_TOP.D2A_RLD_CLK64K !== rand_bit) begin
        `nnc_error("ATM9",$sformatf("[ERROR][%d] D2A_RLD_CLK64K = %b is not as expectation of rand_bit = %b", i, `ANA_TOP.D2A_RLD_CLK64K , rand_bit));
      end
      release `SOC_TB.IOBUF_PAD[16];
      #10000;
    end

    // Checking pin D2A_RLD_EN 
    for (int i=0; i < 100; i++) begin
      force `SOC_TB.IOBUF_PAD[17] = $random;
      #10000;
      rand_bit = `SOC_TB.IOBUF_PAD[17];  
      if (`ANA_TOP.D2A_RLD_EN !== rand_bit) begin
        `nnc_error("ATM9",$sformatf("[ERROR][%d] D2A_RLD_EN = %b is not as expectation of rand_bit = %b", i, `ANA_TOP.D2A_RLD_EN , rand_bit));
      end
