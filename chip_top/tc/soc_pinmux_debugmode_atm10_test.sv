...
//--------------------------------------------------------------------------------------
// File Name	: soc_pinmux_debugmode_atm10_test.sv                                                   
...
class `TESTNAME extends soc_base_test;
  ...
  virtual task main_phase(uvm_phase phase);
    ...
    `nnc_info("SOC_TEST", "soc_pinmux_debugmode_atm10_test start", UVM_LOW)
...
`ifndef FPGA
    ...
    force `SOC_TB.IOBUF_PAD[9:6] = 4'b1011;  
    ...
    if (top_test_cfg.atm !== (13'b1_0000_0000_0000 >> `SOC_TB.IOBUF_PAD[9:6]-1))
      begin
        `nnc_error("ATM10",$sformatf("ATM[0:12] = %b is not as expectation of ATM = %b", top_test_cfg.atm, (13'b1_0000_0000_0000 >> `SOC_TB.IOBUF_PAD[9:6]-1))); 
      end

    if (`ANA_TOP.D2A_BIST_EN !== 1'b0)
      `nnc_error("ATM10",$sformatf("[ERROR] ANA_TOP.D2A_BIST_EN = %b is not as expectation 1'b0", `ANA_TOP.D2A_BIST_EN));

    if (`ANA_TOP.D2A_ECGCAL_EN !== 1'b0)
      `nnc_error("ATM10",$sformatf("[ERROR] ANA_TOP.D2A_ECGCAL_EN = %b is not as expectation 1'b0", `ANA_TOP.D2A_ECGCAL_EN));
    
    // Checking pin D2A_OSC_TRIM 
    for (int i=0; i < 100; i++) begin
      force {`SOC_TB.IOBUF_PAD[0],`SOC_TB.IOBUF_PAD[2],`SOC_TB.IOBUF_PAD[1],`SOC_TB.IOBUF_PAD[3],`SOC_TB.IOBUF_PAD[4], `SOC_TB.IOBUF_PAD[11]} = $random;
      #50000;
      D2A_OSC_TRIM = {`SOC_TB.IOBUF_PAD[0],`SOC_TB.IOBUF_PAD[2],`SOC_TB.IOBUF_PAD[1],`SOC_TB.IOBUF_PAD[3],`SOC_TB.IOBUF_PAD[4], `SOC_TB.IOBUF_PAD[11]};         
      rand_num[5:0] = D2A_OSC_TRIM;  
      if (`ANA_TOP.D2A_OSC_TRIM !== rand_num[5:0]) begin
        `nnc_error("ATM10",$sformatf("[ERROR][%d] D2A_OSC_TRIM = %b is not as expectation of rand_num[5:0] = %b", i, `ANA_TOP.D2A_OSC_TRIM , rand_num[5:0]));
      end
      release {`SOC_TB.IOBUF_PAD[0],`SOC_TB.IOBUF_PAD[2],`SOC_TB.IOBUF_PAD[1],`SOC_TB.IOBUF_PAD[3],`SOC_TB.IOBUF_PAD[4], `SOC_TB.IOBUF_PAD[11]};
      #10000;
    end
    
    // Checking pin A2D_OSC_OUT 
    for (int i=0; i < 100; i++) begin
      force `ANA_TOP.A2D_OSC_OUT = $random;
      #10000;
      rand_bit = `ANA_TOP.A2D_OSC_OUT;  
      if (`SOC_TB.IOBUF_PAD[13] !== rand_bit) begin
        `nnc_error("ATM10",$sformatf("[ERROR][%d] A2D_OSC_OUT = %b is not as expectation of rand_bit = %b", i, `ANA_TOP.A2D_OSC_OUT , rand_bit));
      end
      release `ANA_TOP.A2D_OSC_OUT;
      #10000;
    end

    // Checking pin D2A_OSC_ISEL_TRIM[0] 
    for (int i=0; i < 100; i++) begin
      force `SOC_TB.IOBUF_PAD[12] = $random;
      #10000;
      rand_bit = `SOC_TB.IOBUF_PAD[12];  
      if (`ANA_TOP.D2A_OSC_ISEL_TRIM[0] !== rand_bit) begin
        `nnc_error("ATM10",$sformatf("[ERROR][%d] D2A_OSC_ISEL_TRIM[0] = %b is not as expectation of rand_bit = %b", i, `ANA_TOP.D2A_OSC_ISEL_TRIM[0] , rand_bit));
      end
...
`ifdef POSTLAYOUT
    if(`SOC_TOP.IOBUF_PU[2:0] !== 3'h0)
      begin 
        `nnc_error("ATM0",$sformatf("The value of `SOC_TOP.IOBUF_PU[2:0] = %b is not equal to expected value = %b", `SOC_TOP.IOBUF_PU[2:0], 3'h0));
      end
`elsif POSTSCAN
    if(`SOC_TOP.IOBUF_PU_always_on[2:0] !== 3'h7)
      begin 
        `nnc_error("ATM0",$sformatf("The value of `SOC_TOP.IOBUF_PU_always_on[2:0] = %b is not equal to expected value = %b", `SOC_TOP.IOBUF_PU_always_on[2:0], 3'h7));
      end
`else
