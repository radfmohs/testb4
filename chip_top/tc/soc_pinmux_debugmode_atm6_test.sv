...
class `TESTNAME extends soc_base_test;
  ...
  logic [2:0] D2A_TSC_BJT_SEL;
  logic [2:0] D2A_SDM_VIN_SEL;
  logic [2:0] D2A_TSC_INA_GSEL; 
  ...
  virtual task main_phase(uvm_phase phase);
    ...
    `nnc_info("SOC_TEST", "soc_pinmux_debugmode_atm6_test start", UVM_LOW)
...
`ifndef FPGA
    ...
    force `SOC_TB.IOBUF_PAD[9:6] = 4'b0111;  
    ...
    if (top_test_cfg.atm !== (13'b1_0000_0000_0000 >> `SOC_TB.IOBUF_PAD[9:6]-1))
      begin
        `nnc_error("ATM6",$sformatf("ATM[0:12] = %b is not as expectation of ATM = %b", top_test_cfg.atm, (13'b1_0000_0000_0000 >> `SOC_TB.IOBUF_PAD[9:6]-1))); 
      end
  
    if (`ANA_TOP.D2A_BIST_EN !== 1'b0)
      `nnc_error("ATM6",$sformatf("[ERROR] ANA_TOP.D2A_BIST_EN = %b is not as expectation 1'b0", `ANA_TOP.D2A_BIST_EN));

    if (`ANA_TOP.D2A_ECGCAL_EN !== 1'b0)
      `nnc_error("ATM6",$sformatf("[ERROR] ANA_TOP.D2A_ECGCAL_EN = %b is not as expectation 1'b0", `ANA_TOP.D2A_ECGCAL_EN));
 
    // Checking pin D2A_TSC_BJT_SEL
    for (int i=0; i < 100; i++) begin
      force {`SOC_TB.IOBUF_PAD[5],`SOC_TB.IOBUF_PAD[10],`SOC_TB.IOBUF_PAD[0]} = $random;
      D2A_TSC_BJT_SEL = {`SOC_TB.IOBUF_PAD[5],`SOC_TB.IOBUF_PAD[10],`SOC_TB.IOBUF_PAD[0]};                
      #10000;
      rand_num[2:0] = D2A_TSC_BJT_SEL;
      if (`ANA_TOP.D2A_TSC_BJT_SEL !== rand_num[2:0]) begin
        `nnc_error("ATM6",$sformatf("[ERROR][%d] D2A_TSC_BJT_SEL = %b is not as expectation of rand_num[2:0] = %b", i, `ANA_TOP.D2A_TSC_BJT_SEL , rand_num[2:0]));
      end
    ...
    for (int i=0; i < 100; i++) begin
      ...
      D2A_SDM_VIN_SEL = {`SOC_TB.IOBUF_PAD[2],`SOC_TB.IOBUF_PAD[1],`SOC_TB.IOBUF_PAD[3]};                
      #10000;
      rand_num[2:0] = D2A_SDM_VIN_SEL;
      if (`ANA_TOP.D2A_SDM_VIN_SEL !== rand_num[2:0]) begin
        `nnc_error("ATM6",$sformatf("[ERROR][%d] D2A_SDM_VIN_SEL = %b is not as expectation of rand_num[2:0] = %b", i, `ANA_TOP.D2A_SDM_VIN_SEL , rand_num[2:0]));
      end
    ...
    for (int i=0; i < 100; i++) begin
      force {`SOC_TB.IOBUF_PAD[16],`SOC_TB.IOBUF_PAD[17],`SOC_TB.IOBUF_PAD[18]} = $random;
      D2A_TSC_INA_GSEL = {`SOC_TB.IOBUF_PAD[16],`SOC_TB.IOBUF_PAD[17],`SOC_TB.IOBUF_PAD[18]};                
      #10000;
      rand_num[2:0] = D2A_TSC_INA_GSEL;
      if (`ANA_TOP.D2A_TSC_INA_GSEL !== rand_num[2:0]) begin
        `nnc_error("ATM6",$sformatf("[ERROR][%d] D2A_TSC_INA_GSEL = %b is not as expectation of rand_num[2:0] = %b", i, `ANA_TOP.D2A_TSC_INA_GSEL , rand_num[2:0]));
      end
      release {`SOC_TB.IOBUF_PAD[16],`SOC_TB.IOBUF_PAD[17],`SOC_TB.IOBUF_PAD[18]};
      #10000;
    end
    ...
    for (int i=0; i < 100; i++) begin
      ...
      if (`ANA_TOP.D2A_TSC_EN !== rand_bit) begin
        `nnc_error("ATM6",$sformatf("[ERROR][%d] D2A_TSC_EN = %b is not as expectation of rand_bit = %b", i, `ANA_TOP.D2A_TSC_EN  , rand_bit));
      end
    ...
    for (int i=0; i < 100; i++) begin
      ...
      if (`ANA_TOP.D2A_TSC_OUT_SEL !== rand_bit) begin
        `nnc_error("ATM6",$sformatf("[ERROR][%d] D2A_TSC_OUT_SEL = %b is not as expectation of rand_bit = %b", i, `ANA_TOP.D2A_TSC_OUT_SEL , rand_bit));
      end
    ...
    for (int i=0; i < 100; i++) begin
      ...
      if (`ANA_TOP.D2A_VCM1P5_BUF_EN !== rand_bit) begin
        `nnc_error("ATM6",$sformatf("[ERROR][%d] D2A_VCM1P5_BUF_EN = %b is not as expectation of rand_bit = %b", i, `ANA_TOP.D2A_VCM1P5_BUF_EN , rand_bit));
      end
      release `SOC_TB.IOBUF_PAD[11];
      #10000;
    end   
    
    // Checking pin D2A_TSC_INA_EN 
    for (int i=0; i < 100; i++) begin
      force `SOC_TB.IOBUF_PAD[12] = $random;
      #10000;
      rand_bit = `SOC_TB.IOBUF_PAD[12];  
      if (`ANA_TOP.D2A_TSC_INA_EN !== rand_bit) begin
        `nnc_error("ATM6",$sformatf("[ERROR][%d] D2A_TSC_INA_EN = %b is not as expectation of rand_bit = %b", i, `ANA_TOP.D2A_TSC_INA_EN , rand_bit));
      end
