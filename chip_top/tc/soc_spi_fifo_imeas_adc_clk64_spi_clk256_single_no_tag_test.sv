...
//--------------------------------------------------------------------------------------
// File Name	: soc_spi_fifo_imeas_adc_clk64_spi_clk256_single_no_tag_test.sv                                                   
// Project	: Nanochap ENS3                                  		        
// Description	: Testcase soc_spi_fifo_imeas_adc_clk64_spi_clk256_single_no_tag_test                                             
...
// -------------------------------------------------
`define TESTNAME soc_spi_fifo_imeas_adc_clk64_spi_clk256_single_no_tag_test
`define TESTCFG soc_spi_fifo_imeas_adc_clk64_spi_clk256_single_no_tag_test_cfg
...
class `TESTCFG extends soc_base_test_cfg;
  ...
  function new (string name = "soc_spi_fifo_imeas_adc_clk64_spi_clk256_single_no_tag_test_cfg");
    super.new(name);
  ...
  constraint c_pclk_sel    { soft pclk_sel==0;} //256Khz

  constraint c_iclk_sel    { iclk_sel==1;} //64Khz

  constraint c_spi_sclk_freq          {spi_sclk_freq == 256;};
...
class `TESTNAME extends soc_base_test;
  ...
  virtual task main_phase(uvm_phase phase);
    ...
    `nnc_info("SOC_TEST", "soc_spi_fifo_imeas_adc_clk64_spi_clk256_single_no_tag_test start", UVM_LOW)
...
for (int i = 0; i < loop1; i++) begin
 ...
 repeat (`SOC_FIFO_SIZE) begin
  ...
  if (rd_data_fifo_reg !== `SPI_IF_EXP_DATA) begin
    ...
    `nnc_info(get_full_name(), $sformatf("DATA from IMEAS is received at SPI Master through READ REGISTER is VALUE=%h and Expected Value =%h", rd_data_fifo_reg, `SPI_IF_EXP_DATA),UVM_MEDIUM);
...
/*  
  ...
  `RD_NORMAL_REG(`SOC_FIFO_STATUS_REG,top_test_cfg.pads,rd_data);
    if (rd_data[7] === 1'b1) begin
      ...
      `nnc_error(get_full_name(), $sformatf("FIFO Error happened without expectation"));
    end  
    ...
    // --------------------------------------------------------
    //#10000ns;
    `nnc_info("SOC_TEST", "soc_spi_fifo_imeas_adc_clk64_spi_clk256_single_no_tag_test end now", UVM_LOW)
