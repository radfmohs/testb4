...
//--------------------------------------------------------------------------------------
// File Name	: soc_flash_scanmode_test.sv                                                   
// Project	: Nanochap ENS1P4                                  		        
// Description	: Testcase soc_flash_scanmode_test                                             
// Designer	: ophina@nanochap.com                                                                 
// Date		: 29-11-1623                                                                     
// Revision	: 0.1 Initial version created by script                                 
--------------------------------------------------------------------------------------*/
`define TESTNAME soc_flash_scanmode_test
`define TESTCFG soc_flash_scanmode_test_cfg
`define CLK_PERIOD 3906.25
...
class `TESTCFG extends soc_base_test_cfg;
  ...
  function new (string name = "soc_flash_scanmode_test_cfg");
    super.new(name);    
  endfunction: new
...
class `TESTNAME extends soc_base_test;
  ...
  virtual task main_phase(uvm_phase phase);
    ...
    `nnc_info("SOC_TEST", "soc_flash_scanmode_test start", UVM_LOW)

    // ----------------------------------------------------------------------------------
    // Please add your code of your test here
    // ----------------------------------------------------------------------------------  
    `nnc_info("SOC_TEST",$sformatf("Simulation Begin!!!"),UVM_LOW);
    `nnc_info("SOC_TEST",$sformatf("flash scanmode test"),UVM_LOW);
    wait(`RESETN);
    fork
      gen_clk;
    join_none
  ...
  task gen_clk;    
    forever #(`CLK_PERIOD/2)  scan_clk = ~scan_clk;
  endtask : gen_clk
  ...
  virtual task do_run;
    begin
       ...
       force `SOC_TB.scan_clk = scan_clk;

       `nnc_info("SOC_TEST",$sformatf("\n Select SCANMODE"),UVM_LOW);
       `FLASH_BIST_MASTER_TOP.TESTMODE_SEL = 2'b01;
       ...
       if(`FLASH_TOP.u_32k.CONFEN !== 0) begin
	`nnc_error("SOC_TEST",$sformatf("\tERROR!!! CONFEN set during scanmode!"));
       end
       if(`FLASH_TOP.u_32k.RDN !== 0) begin
	`nnc_error("SOC_TEST",$sformatf("\tERROR!!! RDN set during scanmode!"));
       end
       if(`FLASH_TOP.u_32k.NVR !== 0) begin
	`nnc_error("SOC_TEST",$sformatf("\tERROR!!! NVR set during scanmode!"));
       end
       if(`FLASH_TOP.u_32k.PROG !== 0) begin
	`nnc_error("SOC_TEST",$sformatf("\tERROR!!! PROG set during scanmode!"));
       end
       if(`FLASH_TOP.u_32k.PROG2 !== 0) begin
	`nnc_error("SOC_TEST",$sformatf("\tERROR!!! PROG2 set during scanmode!"));
       end
       if(`FLASH_TOP.u_32k.ERASE !== 0) begin
	`nnc_error("SOC_TEST",$sformatf("\tERROR!!! ERASE set during scanmode!"));
       end
       if(`FLASH_TOP.u_32k.BLOCK !== 0) begin
	`nnc_error("SOC_TEST",$sformatf("\tERROR!!! BLOCK set during scanmode!"));
       end
       if(`FLASH_TOP.u_32k.CHIP !== 0) begin
	`nnc_error("SOC_TEST",$sformatf("\tERROR!!! CHIP set during scanmode!"));
       end
       if(`FLASH_TOP.u_32k.TMEN !== 0) begin
	`nnc_error("SOC_TEST",$sformatf("\tERROR!!! TMEN set during scanmode!"));
       end
       if(`FLASH_TOP.u_32k.VREAD0 !== 0) begin
	`nnc_error("SOC_TEST",$sformatf("\tERROR!!! VREAD0 set during scanmode!"));
       end
       if(`FLASH_TOP.u_32k.VREAD1 !== 0) begin
	`nnc_error("SOC_TEST",$sformatf("\tERROR!!! VREAD1 set during scanmode!"));
       end
       if(`FLASH_TOP.u_32k.VMON1 !== 0) begin
	`nnc_error("SOC_TEST",$sformatf("\tERROR!!! VMON1 set during scanmode!"));
       end
       if(`FLASH_TOP.u_32k.VMON2 !== 0) begin
	`nnc_error("SOC_TEST",$sformatf("\tERROR!!! VMON2 set during scanmode!"));
       end
       if(`FLASH_TOP.u_32k.CEb !== 1) begin
	`nnc_error("SOC_TEST",$sformatf("\tERROR!!! CEb not set during scanmode!"));
       end
       if(`FLASH_TOP.u_32k.WEb !== 1) begin
	`nnc_error("SOC_TEST",$sformatf("\tERROR!!! WEb not set during scanmode!"));
       end
       if(`FLASH_TOP.u_32k.OEb !== 1) begin
	`nnc_error("SOC_TEST",$sformatf("\tERROR!!! OEb not set during scanmode!"));
       end
       if(`FLASH_TOP.u_32k.PORb !== 1) begin
	`nnc_error("SOC_TEST",$sformatf("\tERROR!!! PORb not set during scanmode!"));
       end
       if(`FLASH_TOP.u_32k.DEEPPD !== 1) begin
	`nnc_error("SOC_TEST",$sformatf("\tERROR!!! DEEPPD not set during scanmode!"));
       end
