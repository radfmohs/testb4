...
//--------------------------------------------------------------------------------------
// File Name	: soc_imeas_group_test.sv                                                   
// Project	: Nanochap BAF4P1                                  		        
// Description	: Testcase soc_imeas_group_test                                             
// Designer	: ophina@nanochap.com                                                                 
// Date		: 06-06-2024                                                                     
// Revision	: 0.1 Initial version created by script                                 
--------------------------------------------------------------------------------------*/
`define TESTNAME soc_imeas_group_test
`define TESTCFG soc_imeas_group_test_cfg
...
class `TESTCFG extends soc_base_test_cfg;
  ...
  rand logic [3:0] ch0_num;
  rand logic [3:0] ch1_num;
  rand logic [3:0] ch2_num;
  rand logic       int_en;
       logic [7:0] int_sts;
       logic [11:0] grp_ch_num;
  //------------------------------------------------------------------------------
  //---------------------------- SPI ---------------------------------------------
  rand logic [7:0] mask;
  rand logic [7:0] pads;
  ...
  function new (string name = "soc_imeas_group_test_cfg");
    super.new(name);    
  endfunction: new
  ...
  constraint c_ch_mode        { soft ch_mode == `IMEAS_CHA_MODE_GROUP; }//group channel mode
  // top_test_cfg.top_test_cfg.ch0_num values
  constraint c_ch0_num { ch0_num != ch1_num; ch0_num != ch2_num; ch0_num inside {[0:7]}; }
  // top_test_cfg.top_test_cfg.ch1_num values
  constraint c_ch1_num { ch1_num != ch0_num; ch1_num != ch2_num; ch1_num inside {[0:7]}; }
  // top_test_cfg.top_test_cfg.ch2_num values
  constraint c_ch2_num { ch2_num != ch0_num; ch2_num != ch1_num; ch2_num inside {[0:7]}; }
  ...
  constraint c_cic_rate       { soft cic_rate inside {[0:5]}; }
...
class `TESTNAME extends soc_base_test;
  ...
  virtual task main_phase(uvm_phase phase);
    ...
    `nnc_info("SOC_TEST", "soc_imeas_group_test start", UVM_LOW)

    // ----------------------------------------------------------------------------------
    // Please add your code of your test here
    // ----------------------------------------------------------------------------------  
    `nnc_info("SOC_TEST",$sformatf("Simulation Begin!!!"),UVM_LOW);
    `nnc_info("SOC_TEST",$sformatf("Imeas group channel test"),UVM_LOW);
    do_run;
  ...
  virtual task do_run;
   ...
   //Enable IMEAS
    ...
    ////------------- to write in to the SOC_IMEAS_INPUT_FORMAT reg--------------//
       ...
       top_test_cfg.wr_data[0] = 8'h7F;
    //Set IMEAS grp mode RST COUNT
    ////------------- to write in to the SOC_IMEAS_REG_RSTVAL reg--------------//
       `WR_NORMAL_REG(`SOC_IMEAS_REG_RSTVAL, top_test_cfg.wr_data[0], top_test_cfg.pads);

    top_test_cfg.grp_ch_num = {top_test_cfg.ch2_num,top_test_cfg.ch1_num,top_test_cfg.ch0_num};
    top_test_cfg.wr_data[0] = top_test_cfg.grp_ch_num[7:0];
    //Set SOC_IMEAS_CHA_NUM_LO
    ////------------- to write in to the SOC_IMEAS_CHA_NUM_LO reg--------------// 
       `WR_NORMAL_REG(`SOC_IMEAS_CHA_NUM_LO, top_test_cfg.wr_data[0], top_test_cfg.pads);

    top_test_cfg.wr_data[0] = {4'b0,top_test_cfg.grp_ch_num[11:8]};
    //Set SOC_IMEAS_CHA_NUM_HI
    ////------------- to write in to the SOC_IMEAS_CHA_NUM_HI reg--------------// 
       `WR_NORMAL_REG(`SOC_IMEAS_CHA_NUM_HI, top_test_cfg.wr_data[0], top_test_cfg.pads);

    ///-------------to read from the SOC_IMEAS_CHA_NUM_LO reg-------------------//
       `RD_NORMAL_REG(`SOC_IMEAS_CHA_NUM_LO,top_test_cfg.pads,top_test_cfg.grp_ch_num[7:0]);

   ///-------------to read from the SOC_IMEAS_CHA_NUM_HI reg-------------------//
       `RD_NORMAL_REG(`SOC_IMEAS_CHA_NUM_HI,top_test_cfg.pads,top_test_cfg.grp_ch_num[11:8]);

       `nnc_info("SOC_TEST", $sformatf("select group channel_(%d), channel_(%d), channel_(%d)",top_test_cfg.grp_ch_num[3:0], top_test_cfg.grp_ch_num[7:4], top_test_cfg.grp_ch_num[11:8]),UVM_LOW)
    ...
    ///-------------to read from the SOC_IMEAS_REG_CTRL_0 reg-------------------//
       ...
       top_test_cfg.wr_data[0] = {5'b0,top_test_cfg.format_sel,top_test_cfg.ch_mode};
    ...
    ///-------------to read burst starting from the SOC_IMEAS_CH0DATA_0 reg-------------------//
       ...
       `DUT_IF.ch0_data[7:0]   = top_test_cfg.rd_data_reg[5];
       `DUT_IF.ch0_data[15:8]  = top_test_cfg.rd_data_reg[4];
       `DUT_IF.ch1_data[7:0]   = top_test_cfg.rd_data_reg[3];
       `DUT_IF.ch1_data[15:8]  = top_test_cfg.rd_data_reg[2];
       `DUT_IF.ch2_data[7:0]   = top_test_cfg.rd_data_reg[1];
       `DUT_IF.ch2_data[15:8]  = top_test_cfg.rd_data_reg[0];
    ...
    //Check int status
       if(top_test_cfg.int_sts[0] !== 1'b0)
           `nnc_error("SOC_TEST", "[ERROR][4] imeas interrupt not cleared!")
       else
