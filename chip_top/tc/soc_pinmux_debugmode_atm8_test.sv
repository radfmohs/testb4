...
class `TESTNAME extends soc_base_test;
  ...
  virtual task main_phase(uvm_phase phase);
    ...
    `nnc_info("SOC_TEST", "soc_pinmux_debugmode_atm8_test start", UVM_LOW)
...
`ifndef FPGA
    ...
    force `SOC_TB.IOBUF_PAD[9:6] = 4'b1001;  
    ...
    if (top_test_cfg.atm !== (13'b1_0000_0000_0000 >> `SOC_TB.IOBUF_PAD[9:6]-1))
      begin
        `nnc_error("ATM8",$sformatf("ATM[0:12] = %b is not as expectation of ATM = %b", top_test_cfg.atm, (13'b1_0000_0000_0000 >> `SOC_TB.IOBUF_PAD[9:6]-1))); 
      end

    if (`ANA_TOP.D2A_BIST_EN !== 1'b0)
      `nnc_error("ATM8",$sformatf("[ERROR] ANA_TOP.D2A_BIST_EN = %b is not as expectation 1'b0", `ANA_TOP.D2A_BIST_EN));

    //if (`ANA_TOP.D2A_ECGCAL_EN !== 1'b0)
    //  `nnc_error("ATM8",$sformatf("[ERROR] ANA_TOP.D2A_ECGCAL_EN = %b is not as expectation 1'b0", `ANA_TOP.D2A_ECGCAL_EN));

    // Checking pin D2A_ECGCAL_VSEL_TRIM 
    for (int i=0; i < 100; i++) begin
      force {`SOC_TB.IOBUF_PAD[16], `SOC_TB.IOBUF_PAD[17], `SOC_TB.IOBUF_PAD[5], `SOC_TB.IOBUF_PAD[10], `SOC_TB.IOBUF_PAD[0], `SOC_TB.IOBUF_PAD[2], `SOC_TB.IOBUF_PAD[1], `SOC_TB.IOBUF_PAD[3], `SOC_TB.IOBUF_PAD[4], `SOC_TB.IOBUF_PAD[11]} = $random;
      #10000;
      rand_num[9:0] = {`SOC_TB.IOBUF_PAD[16], `SOC_TB.IOBUF_PAD[17], `SOC_TB.IOBUF_PAD[5], `SOC_TB.IOBUF_PAD[10], `SOC_TB.IOBUF_PAD[0], `SOC_TB.IOBUF_PAD[2], `SOC_TB.IOBUF_PAD[1], `SOC_TB.IOBUF_PAD[3], `SOC_TB.IOBUF_PAD[4], `SOC_TB.IOBUF_PAD[11]};     
      if (`ANA_TOP.D2A_ECGCAL_VSEL_TRIM !== rand_num[9:0]) begin
        `nnc_error("ATM8",$sformatf("[ERROR][%d] D2A_ECGCAL_VSEL_TRIM = %b is not as expectation of rand_num[9:0] = %b", i, `ANA_TOP.D2A_ECGCAL_VSEL_TRIM , rand_num[9:0]));
      end
      release {`SOC_TB.IOBUF_PAD[16], `SOC_TB.IOBUF_PAD[17], `SOC_TB.IOBUF_PAD[5], `SOC_TB.IOBUF_PAD[10], `SOC_TB.IOBUF_PAD[0], `SOC_TB.IOBUF_PAD[2], `SOC_TB.IOBUF_PAD[1], `SOC_TB.IOBUF_PAD[3], `SOC_TB.IOBUF_PAD[4], `SOC_TB.IOBUF_PAD[11]};
      #10000;
    end
    ...
    for (int i=0; i < 100; i++) begin
      ...
      if (`ANA_TOP.D2A_VCM1P5_BUF_EN !== rand_bit) begin
        `nnc_error("ATM8",$sformatf("[ERROR][%d] D2A_VCM1P5_BUF_EN = %b is not as expectation of rand_bit = %b", i, `ANA_TOP.D2A_VCM1P5_BUF_EN , rand_bit));
      end
      release `SOC_TB.IOBUF_PAD[12];
      #10000;
    end

    // Checking pin D2A_ECGCAL_EN 
    for (int i=0; i < 100; i++) begin
      force `SOC_TB.IOBUF_PAD[15] = $random;
      #10000;
      rand_bit = `SOC_TB.IOBUF_PAD[15];     
      if (`ANA_TOP.D2A_ECGCAL_EN !== rand_bit) begin
        `nnc_error("ATM8",$sformatf("[ERROR][%d] D2A_ECGCAL_EN = %b is not as expectation of rand_bit = %b", i, `ANA_TOP.D2A_ECGCAL_EN , rand_bit));
      end
