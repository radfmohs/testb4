...
//--------------------------------------------------------------------------------------
// File Name	: soc_flash_bist_nvr_word_test.sv                                                   
// Project	: Nanochap ENS3                                  		        
// Description	: Testcase soc_flash_bist_nvr_word_test                                             
...
// -------------------------------------------------
`define TESTNAME soc_flash_bist_nvr_word_test
`define TESTCFG soc_flash_bist_nvr_word_test_cfg
...
class `TESTCFG extends soc_base_test_cfg;
  ...
  function new (string name = "soc_flash_bist_nvr_word_test_cfg");
    super.new(name);
...
class `TESTNAME extends soc_base_test;
  ...
  virtual task main_phase(uvm_phase phase);
    ...
    `nnc_info("SOC_TEST", "soc_flash_bist_nvr_word_test start", UVM_LOW)
    ...
    // --------------------------------------------------------
    #10000ns;
    `nnc_info("SOC_TEST", "soc_flash_bist_nvr_word_test end now", UVM_LOW)
...
for (integer i=0; i<512; i++) begin
  ...
  `uvm_info("", $sformatf("[FLASH BIST MASTER][%d] Sending Read Command to read Main  of Flash", i), UVM_LOW);
  `FLASH_BIST_MASTER_TOP.prgm_word_NVR_10M(i, i);
  `uvm_info("", "[FLASH BIST MASTER] Complete successully this phase", UVM_LOW);

  `uvm_info("", $sformatf("[FLASH BIST MASTER][%d] Sending Read Command to read and check Flash", i), UVM_LOW);
  `FLASH_BIST_MASTER_TOP.read_nvr_word_10M(i, top_test_cfg.rd_data[i]);
  `uvm_info("", "[FLASH BIST MASTER] Complete successully this phase", UVM_LOW);
end
...
//release `ANA_TOP.A2D_SW_Z_ADC_EOC;
