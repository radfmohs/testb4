...
//--------------------------------------------------------------------------------------
// File Name	: soc_spi_fifo_imeas_single_mode_disable_tag_test.sv                                                   
// Project	: Nanochap ENS3                                  		        
// Description	: Testcase soc_spi_fifo_imeas_single_mode_disable_tag_test                                             
...
// -------------------------------------------------
`define TESTNAME soc_spi_fifo_imeas_single_mode_disable_tag_test
`define TESTCFG soc_spi_fifo_imeas_single_mode_disable_tag_test_cfg
...
class `TESTCFG extends soc_base_test_cfg;
  ...
  function new (string name = "soc_spi_fifo_imeas_single_mode_disable_tag_test_cfg");
    super.new(name);
...
class `TESTNAME extends soc_base_test;
  ...
  virtual task main_phase(uvm_phase phase);
    ...
    `nnc_info("SOC_TEST", "soc_spi_fifo_imeas_single_mode_disable_tag_test start", UVM_LOW)

    // ==================================================================================
    // Please add your code of your test here
    // ---------------------------------------------------------------------------------- 

    `nnc_info(get_name(), "step-1 Enable IMEAS ", UVM_LOW)
    // Enable IMEAS
    ...
    // TEST with NO TAGs
    // ----------------------------------------
    `nnc_info(get_name(), "step-2  TEST with NO TAGs ", UVM_LOW)
    ...
    top_test_cfg.status_bit = rd_data[4];
    ...
    if (rd_data !== 8'h00) begin
      ...
      `nnc_error("FIFO_READ_POINTER is CURRENT VALUE=%h ,expected=8'h00",rd_data);
    end
    
    `nnc_info(get_name(), "step-3 ", UVM_LOW)
...
`endif
    ...
    for (int i = 0; i < loop1; i++) begin
      ...
      `nnc_info(get_name(), "step-3.3 ", UVM_LOW)
      repeat (`SOC_FIFO_SIZE) begin
       `nnc_info(get_name(), "step-3.4 ", UVM_LOW)
        //wait(!spi_top_u.i_fifo_empty)
        // wait FIFO almost full
        `RD_NORMAL_REG(`SOC_FIFO_STATUS_REG,top_test_cfg.pads,rd_data);
        while (!rd_data[5]) begin
          `RD_NORMAL_REG(`SOC_FIFO_STATUS_REG,top_test_cfg.pads,rd_data);
        end
       `nnc_info(get_name(), "step-3.5 ", UVM_LOW)
        // Read through FIFO_DATA_REG to get the values of DATA in registers
        top_test_cfg.no_of_bytes = 2;
        top_test_cfg.rd_data_reg = new[top_test_cfg.no_of_bytes];
        `RD_BURST_NORMAL_REG(`SOC_FIFO_DATA_REG1, top_test_cfg.no_of_bytes, top_test_cfg.rd_data_reg);
        ...
        top_test_cfg.rd_data_fifo_reg = {rd_data[1:0], top_test_cfg.rd_data_reg[1], top_test_cfg.rd_data_reg[0]};
    
        if (top_test_cfg.rd_data_fifo_reg !== `SPI_IF_EXP_DATA) begin
          //$display("[ERROR][3] DATA from IMEAS is received at SPI Master through READ REGISTER is VALUE=%h != EXPECTED VALUE=%h", top_test_cfg.rd_data_fifo_reg, `SPI_IF_EXP_DATA);
	  `nnc_error(get_full_name(), $sformatf("DATA from IMEAS is received at SPI Master through READ REGISTER is VALUE=%h != EXPECTED VALUE=%h", top_test_cfg.rd_data_fifo_reg, `SPI_IF_EXP_DATA));
        end
        else
          //$display("[SPI INFO] DATA from IMEAS is received at SPI Master through READ REGISTER is VALUE=%h and Expected Value =%h At time = %t", top_test_cfg.rd_data_fifo_reg, `SPI_IF_EXP_DATA, $time);
	  `nnc_info(get_full_name(), $sformatf("DATA from IMEAS is received at SPI Master through READ REGISTER is VALUE=%h != EXPECTED VALUE=%h", top_test_cfg.rd_data_fifo_reg, `SPI_IF_EXP_DATA),UVM_DEBUG);
        ...
        top_test_cfg.rd_counter = top_test_cfg.rd_counter + top_test_cfg.no_of_elements;
	repeat(2) @(posedge `DUT_IF.sys_clk);
    
        // Checking FIFO RD POINTER
        `RD_NORMAL_REG(`SOC_FIFO_RD_PTR_REG,top_test_cfg.pads,rd_data);
        // Checking Read pointer must be not 0
        if (rd_data !== top_test_cfg.rd_counter) begin
          //$display("[ERROR][4] FIFO_READ_POINTER is CURRENT VALUE=%h != EXPECTED VALUE=%h at TIME = %t ", rd_data, top_test_cfg.rd_counter, $time);
	  `nnc_error(get_full_name(), $sformatf("FIFO_READ_POINTER is CURRENT VALUE=%h != EXPECTED VALUE=%h", rd_data, top_test_cfg.rd_counter));
          //`nnc_error(" FIFO_READ_POINTER is CURRENT VALUE=%h != EXPECTED VALUE=%h at TIME = %t ", rd_data, top_test_cfg.rd_counter);
        end
    
        // Compare DATA read in registers and from FIFO data bus
        for (int i = 0; i < top_test_cfg.no_of_elements; i++) begin
          if (top_test_cfg.rd_data_fifo_reg[15:0] !== top_test_cfg.rd_data_fifo[i]) begin
            //$display("[ERROR][5] READ DATA from FIFO Register Element-%d: %h is NOT EQUAL to THE VALUE READ DIRECT from FIFO: %h", i, top_test_cfg.rd_data_fifo_reg[15:0], top_test_cfg.rd_data_fifo[i]);
	    `nnc_error(get_full_name(), $sformatf("READ DATA from FIFO Register Element-%d: %h is NOT EQUAL to THE VALUE READ DIRECT from FIFO: %h", i, top_test_cfg.rd_data_fifo_reg[15:0], top_test_cfg.rd_data_fifo[i]));
            //`nnc_error("READ DATA from FIFO Register Element-%d: %h is NOT EQUAL to THE VALUE READ DIRECT from FIFO: %h", i, top_test_cfg.rd_data_fifo_reg[15:0], top_test_cfg.rd_data_fifo[i]);
          end
        end
...
/*
        // Extract from FIFO
        top_test_cfg.rd_data_imeas2fifo = top_cfg.imeas2fifo_data_mem.pop_front();
        // Compare DATA read in registers and from FIFO data bus
        for (int i = 0; i < top_test_cfg.no_of_elements; i++) begin
          if (top_test_cfg.rd_data_imeas2fifo[15:0] !== top_test_cfg.rd_data_fifo[i]) begin
            $display("[ERROR][6] READ DATA from FIFO Element-%d: %h is NOT EQUAL to THE VALUE from IMEAS to FIFO: %h", i, top_test_cfg.rd_data_imeas2fifo[15:0], top_test_cfg.rd_data_fifo[i]);
	    `nnc_error(get_full_name(),"MISMATCH in DATA");
            //`nnc_error("READ DATA from FIFO Element-%d: %h is NOT EQUAL to THE VALUE from IMEAS to FIFO: %h", i, top_test_cfg.rd_data_imeas2fifo[15:0], top_test_cfg.rd_data_fifo[i]);
          end
        end */
      end
      `nnc_info(get_name(), "step-3.6 ", UVM_LOW)
    ...
    `nnc_info(get_name(), "step-4 ", UVM_LOW)
   ...
   /*   
      ...
      for (int i = 0; i < top_test_cfg.no_of_elements; i++) begin
          // Extract from FIFO
        top_test_cfg.rd_data_imeas2fifo = top_cfg.imeas2fifo_data_mem.pop_front();
        if (top_test_cfg.rd_data_imeas2fifo[15:0] !== top_test_cfg.rd_data_fifo[top_test_cfg.no_of_elements - 1 - i]) begin
          $display("[ERROR][6] READ DATA from FIFO Element-%d: %h is NOT EQUAL to THE VALUE from IMEAS to FIFO: %h", i, top_test_cfg.rd_data_imeas2fifo[15:0], top_test_cfg.rd_data_fifo[top_test_cfg.no_of_elements - 1 - i]);
	  `nnc_error(get_full_name(),"MISMATCH in DATA");
	//  `nnc_error("READ DATA from FIFO Element-%d: %h is NOT EQUAL to THE VALUE from IMEAS to FIFO: %h", i, top_test_cfg.rd_data_imeas2fifo[15:0], top_test_cfg.rd_data_fifo[top_test_cfg.no_of_elements - 1 - i]);
        end
      end
      */
      `nnc_info(get_name(), "step-5 ", UVM_LOW)
      // Get TAGs from Status register
      `RD_NORMAL_REG(`SOC_FIFO_STATUS_REG,top_test_cfg.pads,rd_data);
    ...
    // --------------------------------------------------------
    //#10000ns;
    `nnc_info("SOC_TEST", "soc_spi_fifo_imeas_single_mode_disable_tag_test end now", UVM_LOW)
