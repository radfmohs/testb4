...
//--------------------------------------------------------------------------------------
// File Name	: soc_spi_fifo_imeas_work_around_test.sv                                                   
// Project	: Nanochap ENS3                                  		        
// Description	: Testcase soc_spi_fifo_imeas_work_around_test                                             
...
// -------------------------------------------------
`define TESTNAME soc_spi_fifo_imeas_work_around_test
`define TESTCFG soc_spi_fifo_imeas_work_around_test_cfg
...
class `TESTCFG extends soc_base_test_cfg;
  ...
  function new (string name = "soc_spi_fifo_imeas_work_around_test_cfg");
    super.new(name);
  ...
  constraint c_spi_sclk_freq          { solve pclk_sel before spi_sclk_freq; spi_sclk_freq inside {[64 : 256/(2**pclk_sel)*4]};}
...
class `TESTNAME extends soc_base_test;
  ...
  virtual task main_phase(uvm_phase phase);
    ...
    `nnc_info("SOC_TEST", "soc_spi_fifo_imeas_work_around_test start", UVM_LOW)

    // ==================================================================================
    // Please add your code of your test here
    // ---------------------------------------------------------------------------------- 
    
// Change ADC clock from 256KHz to 64KHz - set bit[5:3] to 3'b010 256/4=64Khz
`RD_NORMAL_REG(`SOC_CLK_CTRL_REG,top_test_cfg.pads,rd_data);
rd_data[5:3] = 3'b001;
wr_data = rd_data;
`WR_NORMAL_REG(`SOC_CLK_CTRL_REG, wr_data, top_test_cfg.pads);

// Enable WRCMD to Flash
//wr_data = 8'h03; 
//`WR_RD_CHK_NORMAL_REG(`SOC_FLASH_CONFIG,wr_data,top_test_cfg.pads,top_test_cfg.mask);

// Change ADC clock from 256KHz to 64KHz - set bit[6:4] to 3'b000
`RD_NORMAL_REG(`SOC_IMEAS_REG_CTRL_0,top_test_cfg.pads,rd_data);
wr_data = rd_data & 8'b1000_1111;
`WR_NORMAL_REG(`SOC_IMEAS_REG_CTRL_0, wr_data, top_test_cfg.pads);
...
for (int i = 0; i < loop1; i++) begin
  wait(top_cfg.imeas2fifo_mem_cnt === 256*(i+1));
 ...
 repeat (256) begin
  ...
  if (rd_data_fifo_reg !== `SPI_IF_EXP_DATA) begin
    ...
    `nnc_info(get_full_name(), $sformatf("DATA from IMEAS is received at SPI Master through READ REGISTER is VALUE=%h != EXPECTED VALUE=%h", rd_data_fifo_reg, `SPI_IF_EXP_DATA),UVM_MEDIUM);
  ...
  for (int i = 0; i < top_test_cfg.no_of_elements; i++) begin
    if (rd_data_fifo_reg[15:0] !== rd_data_fifo[i]) begin
      ...
      `nnc_error(get_full_name(), $sformatf("READ DATA from FIFO Register Element-%d: %h is NOT EQUAL to THE VALUE READ DIRECT from FIFO: %h", i, rd_data_fifo_reg[15:0], rd_data_fifo[i]));
    end
  end
...
/*
  ...
  for (int i = 0; i < top_test_cfg.no_of_elements; i++) begin
    if (rd_data_imeas2fifo[15:0] !== rd_data_fifo[i]) begin
      //$display("[ERROR][6] READ DATA from FIFO Element-%d: %h is NOT EQUAL to THE VALUE from IMEAS to FIFO: %h", i, rd_data_imeas2fifo[15:0], rd_data_fifo[i]);
      `nnc_error(get_full_name(), $sformatf("READ DATA from FIFO Element-%d: %h is NOT EQUAL to THE VALUE from IMEAS to FIFO: %h", i, rd_data_fifo_reg[15:0], rd_data_fifo[i]));
    end
  end */
 end
...
rd_data_fifo = new[1024];
for (int i=0; i < 3; i++) begin
 
  while (rd_data<8'h7F)
  `RD_NORMAL_REG(`SOC_FIFO_COUNTER_2_REG,top_test_cfg.pads,rd_data);

/*
 // Read a large burst from FIFO
  top_test_cfg.no_of_elements = rd_data;
  ...
  for (int i = 0; i < top_test_cfg.no_of_elements; i++) begin
    ...
    if (rd_data_imeas2fifo[15:0] !== rd_data_fifo[top_test_cfg.no_of_elements - 1 - i]) begin
      //$display("[ERROR][6] READ DATA from FIFO Element-%d: %h is NOT EQUAL to THE VALUE from IMEAS to FIFO: %h at time: %t", top_test_cfg.no_of_elements - 1 - i, rd_data_fifo[top_test_cfg.no_of_elements - 1 - i], rd_data_imeas2fifo[15:0], $time);
      `nnc_error(get_full_name(), $sformatf("READ DATA from FIFO Element-%d: %h is NOT EQUAL to THE VALUE from IMEAS to FIFO: %h", top_test_cfg.no_of_elements - 1 - i, rd_data_fifo[top_test_cfg.no_of_elements - 1 - i], rd_data_imeas2fifo[15:0]));
    end
  end*/
  ...
  `RD_NORMAL_REG(`SOC_FIFO_STATUS_REG,top_test_cfg.pads,rd_data);
    if (rd_data[7] === 1'b1) begin
      //$display("[ERROR][7] FIFO Error happened without expectation at time: %t", $time);
      `nnc_error(get_full_name(),"FIFO Error happened without expectation");
    end  
    ...
    // --------------------------------------------------------
    //#10000ns;
    `nnc_info("SOC_TEST", "soc_spi_fifo_imeas_work_around_test end now", UVM_LOW)
