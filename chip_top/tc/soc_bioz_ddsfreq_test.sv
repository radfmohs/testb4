...
--------------------------------------------------------------------------------------*/
`define TESTNAME soc_bioz_ddsfreq_test
`define TESTCFG soc_bioz_ddsfreq_test_cfg
...
class `TESTCFG extends soc_base_test_cfg;
  ...
  rand logic [31:0] phase_offset;
  rand logic [31:0] phase_offset_c;
  //---------------------------- FLASH -------------------------------------------
  ...
  function new (string name = "soc_bioz_ddsfreq_test_cfg");
    super.new(name);    
  endfunction: new
...
class `TESTNAME extends soc_bioz_base_test;
  ...
  virtual task do_run;
   ...
   //Enable ZMEAS
    ////------------- to write in to the SOC_ZMEAS_EN reg--------------//
       `WR_NORMAL_REG(`SOC_BIOZ_CTRL, top_test_cfg.wr_data[0], top_test_cfg.pads);
       `WR_NORMAL_REG(`SOC_FLASH_UNLOCK_PRO, 8'hAA, top_test_cfg.pads);

    ///-------------to read from the SOC_ZMEAS_EN reg-------------------//
       `RD_NORMAL_REG(`SOC_BIOZ_CTRL,top_test_cfg.pads,top_test_cfg.rd_data);
...
*/
        bioz_state_change(0);
        set_ddsfreq(top_test_cfg.phase_inc, top_test_cfg.phase_offset, top_test_cfg.phase_offset_c);
        bioz_state_change(0);
       //#1ns;
    end
  endtask
