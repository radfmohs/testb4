...
//--------------------------------------------------------------------------------------
// File Name	: soc_spi_fifo_roll_flush_debug_interrupt_test.sv                                                   
// Project	: Nanochap ENS3                                  		        
// Description	: Testcase soc_spi_fifo_roll_flush_debug_interrupt_test                                             
...
// -------------------------------------------------
`define TESTNAME soc_spi_fifo_roll_flush_debug_interrupt_test
`define TESTCFG soc_spi_fifo_roll_flush_debug_interrupt_test_cfg
...
class `TESTCFG extends soc_base_test_cfg;
  ...
  function new (string name = "soc_spi_fifo_roll_flush_debug_interrupt_test_cfg");
    super.new(name);
...
class `TESTNAME extends soc_base_test;
  ...
  virtual task main_phase(uvm_phase phase);
    ...
    `nnc_info("SOC_TEST", "soc_spi_fifo_roll_flush_debug_interrupt_test start", UVM_LOW)
...
wr_data = 8'hC0; // Defaulf of A_FULL_LEVEF is 8'h5F
...
$display("[INFO] STEP 11 Checking is DONE at %t",$time);
// ---------------------------------------------------------------------
...
$display("[INFO] STEP 12 Checking is DONE at %t",$time);
// ---------------------------------------------------------------------
...
$display("[INFO] - Disable IMEAS at TIME: %t", $time);

// Disbale IMEAS
wr_data = 8'h00; 
`WR_NORMAL_REG(`SOC_IMEAS_EN,wr_data,top_test_cfg.pads);

$display("[INFO] STEP 13 Checking is DONE at %t",$time);
// ---------------------------------------------------------------------
...
$display("[INFO] STEP 14 Checking is DONE at %t",$time);
// ---------------------------------------------------------------------
...
if ({rd_data_reg[1][7],rd_data_reg[0]} !== (9'h100 - top_test_cfg.no_of_elements - top_test_cfg.no_of_elements)) begin
  //$display("[ERROR][TEST][14] FIFO_DATA_COUNT is CURRENT VALUE=%h != EXPECTED VALUE=%h at TIME = %t", {rd_data_reg[1][7],rd_data_reg[0]}, (9'h100 - top_test_cfg.no_of_elements - top_test_cfg.no_of_elements), $time);
  `nnc_error(get_full_name(), $sformatf("FIFO_DATA_COUNT is CURRENT VALUE=%h != EXPECTED VALUE=%h", {rd_data_reg[1][7],rd_data_reg[0]}, (9'h100 - top_test_cfg.no_of_elements - top_test_cfg.no_of_elements)));
end
...
`WR_RD_CHK_NORMAL_REG(`SOC_FIFO_CONFIG_3_REG, wr_data, top_test_cfg.pads, top_test_cfg.mask);
    ...
    // --------------------------------------------------------
    //#10000ns;
    `nnc_info("SOC_TEST", "soc_spi_fifo_roll_flush_debug_interrupt_test end now", UVM_LOW)
