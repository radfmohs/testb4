...
//--------------------------------------------------------------------------------------
// File Name	: soc_imeas_continous_alarm_test.sv                                                   
// Project	: Nanochap BAF4P1                                  		        
// Description	: Testcase soc_imeas_continous_alarm_test                                             
// Designer	: ophina@nanochap.com                                                                 
// Date		: 06-06-2024                                                                     
// Revision	: 0.1 Initial version created by script                                 
--------------------------------------------------------------------------------------*/
`define TESTNAME soc_imeas_continous_alarm_test
`define TESTCFG soc_imeas_continous_alarm_test_cfg
...
class `TESTCFG extends soc_base_test_cfg;
  ...
  function new (string name = "soc_imeas_continous_alarm_test_cfg");
    super.new(name);    
  endfunction: new
  ...
  constraint c_ch_mode        { soft ch_mode == `IMEAS_CHA_MODE_SINGLE_CONT; }//single continous channel mode
...
class `TESTNAME extends soc_base_test;
  ...
  virtual task main_phase(uvm_phase phase);
    ...
    `nnc_info("SOC_TEST", "soc_imeas_continous_alarm_test start", UVM_LOW)

    // ----------------------------------------------------------------------------------
    // Please add your code of your test here
    // ----------------------------------------------------------------------------------  
    `nnc_info("SOC_TEST",$sformatf("Simulation Begin!!!"),UVM_LOW);
    `nnc_info("SOC_TEST",$sformatf("Imeas single continous channel alarm test"),UVM_LOW);
    do_run;
  ...
  virtual task do_run;
    begin
       ...
       top_test_cfg.wr_data[0] = 8'h30;
    //Set imeas clk div
    ////------------- to write in to the SOC_CLK_CTRL_REG reg--------------//
       `WR_NORMAL_REG(`SOC_CLK_CTRL_REG, top_test_cfg.wr_data[0], top_test_cfg.pads);//set adc_clk 2KHz
   ...
   //Enable IMEAS
    ...
    ///-------------to read from the SOC_IMEAS_REG_CTRL_0 reg-------------------//
       ...
       #1000;
       //alarm_th_lo = 16'h20A0;
       top_test_cfg.wr_data[0] = 8'hA0;
       top_test_cfg.wr_data[1] = 8'h20;
     ...
     ////------------- to write in to the SOC_IMEAS_THRESHOLD_LO_1 reg--------------//
       ...
       //alarm_th_hi = 16'h60A0;
       top_test_cfg.wr_data[0] = 8'hA0;
       top_test_cfg.wr_data[1] = 8'h60;
    ...
    ////------------- to write in to the SOC_IMEAS_STATUS reg--------------//
       `WR_NORMAL_REG(`SOC_IMEAS_STATUS, top_test_cfg.wr_data[0], top_test_cfg.pads);//interrupt clear
    ...
    ///-------------to read from the SOC_IMEAS_ALARM_INT reg-------------------//
       ...
       if(`DUT_IF.alarm_sts === 1'b1) begin
          `nnc_info("SOC_TEST", "imeas alarm interrupt occured!", UVM_LOW)
