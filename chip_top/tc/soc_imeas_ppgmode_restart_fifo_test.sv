...
//--------------------------------------------------------------------------------------
// File Name	: soc_imeas_ppgmode_restart_fifo_test.sv                                                   
// Project	: Nanochap BAF4P1                                  		        
// Description	: Testcase soc_imeas_ppgmode_restart_fifo_test                                             
// Designer	: ophina@nanochap.com                                                                 
// Date		: 06-06-2024                                                                     
// Revision	: 0.1 Initial version created by script                                 
--------------------------------------------------------------------------------------*/
`define TESTNAME soc_imeas_ppgmode_restart_fifo_test
`define TESTCFG soc_imeas_ppgmode_restart_fifo_test_cfg
...
class `TESTCFG extends soc_base_test_cfg;
  ...
  function new (string name = "soc_imeas_ppgmode_restart_fifo_test_cfg");
    super.new(name);    
  endfunction: new
...
class `TESTNAME extends soc_base_test;
  ...
  virtual task main_phase(uvm_phase phase);
    ...
    `nnc_info("SOC_TEST", "soc_imeas_ppgmode_restart_fifo_test start", UVM_LOW)

    // ----------------------------------------------------------------------------------
    // Please add your code of your test here
    // ----------------------------------------------------------------------------------  
    `nnc_info("SOC_TEST",$sformatf("Simulation Begin!!!"),UVM_LOW);
    `nnc_info("SOC_TEST",$sformatf("Imeas single channel ppg mode fifo test with restart enabled"),UVM_LOW);
    fork
      collect_data();
    join_none
    do_run();
  ...
  virtual task do_run;
    begin
      ...
      top_test_cfg.wr_data[0] = 8'h10;
      //Set imeas clk div
      ////------------- to write in to the SOC_CLK_CTRL_REG reg--------------//
      `WR_NORMAL_REG(`SOC_CLK_CTRL_REG, top_test_cfg.wr_data[0], top_test_cfg.pads);//set adc_clk 32KHz
      ...
      top_test_cfg.wr_data[0] = {1'b1,top_test_cfg.cic_rate,3'b1,top_test_cfg.int_en};
...
`endif
      if(!`ANA_TOP.D2A_PPG_SH_CK&&!`SOC_TB.IOBUF_PAD[18] || `ANA_TOP.D2A_PPG_SH_CK&&`SOC_TB.IOBUF_PAD[18]) begin
         ...
         top_test_cfg.ch0_len = top_test_cfg.ch0_len + 1;
         if(`IMEAS_TOP.u_imeas_cic_0.eoc_out === 0) begin
