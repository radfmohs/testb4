...
//--------------------------------------------------------------------------------------
// File Name	: soc_zmeas_norepeat_freq4k_test.sv                                                   
// Project	: Nanochap BAF4P1                                  		        
// Description	: Testcase soc_zmeas_norepeat_freq4k_test                                             
// Designer	: ophina@nanochap.com                                                                 
// Date		: 06-06-2024                                                                     
// Revision	: 0.1 Initial version created by script                                 
--------------------------------------------------------------------------------------*/
`define TESTNAME soc_zmeas_norepeat_freq4k_test
`define TESTCFG soc_zmeas_norepeat_freq4k_test_cfg
...
class `TESTCFG extends soc_base_test_cfg;
  ...
  function new (string name = "soc_zmeas_norepeat_freq4k_test_cfg");
    super.new(name);    
  endfunction: new
  ...
  constraint c_pclk_sel       { soft pclk_sel == 0; }//adc_eoc capture to adc_int_sts require pclk=mclk
  ...
  constraint c_freq_val       { soft freq_val == `ZMEAS_REG_FREQ_VAL_4KHZ; }//freq_val=4KHz
  // top_test_cfg.top_test_cfg.zmeas_freq_sel values
  constraint c_zmeas_freq_sel { soft zmeas_freq_sel == 2'b10; }//freq_val=4KHz
...
class `TESTNAME extends soc_base_test;
  ...
  virtual task main_phase(uvm_phase phase);
    ...
    `nnc_info("SOC_TEST", "soc_zmeas_norepeat_freq4k_test start", UVM_LOW)

    // ----------------------------------------------------------------------------------
    // Please add your code of your test here
    // ----------------------------------------------------------------------------------  
    `nnc_info("SOC_TEST",$sformatf("Simulation Begin!!!"),UVM_LOW);
    `nnc_info("SOC_TEST",$sformatf("ZMEAS test without repeat(freq_val:4KHz)"),UVM_LOW);
    do_run;
  ...
  virtual task do_run;
   ...
   //Enable ZMEAS
    ...
    //Check Device status
      ///-------------to read from the SOC_DEVICE_INT_STATUS_0 reg-------------------//
      `RD_NORMAL_REG(`SOC_DEVICE_INT_STATUS_0,top_test_cfg.pads,top_test_cfg.rd_data);
       if(top_test_cfg.rd_data[1:0] !== 2'b11)
          `nnc_error("SOC_TEST", "[ERROR][2] Device int status not expected!")
    ...
    ///-------------to read from the SOC_ZMEAS_REG_STATUS_1 reg-------------------//
       ...
       if(top_test_cfg.status[15:12] !== top_test_cfg.repeat_num)
         `nnc_error("SOC_TEST", "[ERROR][3] zmeas_status[15:12] is not expected!")
       if(top_test_cfg.status[11:9] !== top_test_cfg.freq_val)
         `nnc_error("SOC_TEST", "[ERROR][4] zmeas_status[11:9] is not expected!")
       if((top_test_cfg.status[8:7] !== {top_test_cfg.zmeas_analog_en,top_test_cfg.tx_gain_sel[0]}) || (top_test_cfg.status[8:7] !== {`ANA_TOP.D2A_SW_Z_EN,`ANA_TOP.D2A_SW_Z_TX_GSEL[0]}))
         `nnc_error("SOC_TEST", "[ERROR][5] zmeas_status[8:7] is not expected!")
       if((top_test_cfg.status[6] !== top_test_cfg.calib) || (top_test_cfg.status[6] !== `ANA_TOP.D2A_SW_Z_CALMD))
         `nnc_error("SOC_TEST", "[ERROR][6] zmeas_status[6] is not expected!")
       if((top_test_cfg.status[5] !== top_test_cfg.rx_pga_gain) || (top_test_cfg.status[5] !== `ANA_TOP.D2A_SW_Z_RXGAIN))
         `nnc_error("SOC_TEST", "[ERROR][7] zmeas_status[5] is not expected!")
       if(top_test_cfg.status[4:3] !== 2'b0)//adc_en,dds_en
         `nnc_error("SOC_TEST", "[ERROR][8] zmeas_status[4:3] is not expected!")
       if(top_test_cfg.status[2:0] !== 7) //measurement is done
         `nnc_error("SOC_TEST", "[ERROR][9] zmeas_status[2:0] is not expected!")
    ...
    ///-------------to read burst starting from the SOC_ZMEAS_REG_DATAOUT_0 reg-------------------//
       ...
       if(top_test_cfg.zmeas_data !== 32'h0008f83c)
    ...
    //Check int status
       if(top_test_cfg.int_sts[0] !== 1'b0)
           `nnc_error("SOC_TEST", "[ERROR][11] zmeas interrupt not cleared!")
       else
	   `nnc_info("SOC_TEST", "zmeas interrupt cleared!", UVM_LOW)
    //Check Device status
      ///-------------to read from the SOC_DEVICE_INT_STATUS_0 reg-------------------//
       `RD_NORMAL_REG(`SOC_DEVICE_INT_STATUS_0,top_test_cfg.pads,top_test_cfg.rd_data);
       if(top_test_cfg.rd_data[1:0] !== 2'b01)
	 `nnc_error("SOC_TEST", "[ERROR][12] Device int status not expected!")

     //Clear adc interrupt status
     ///-------------to read from the SOC_ZMEAS_SUM_SHIFT_0 reg-------------------//
       `RD_NORMAL_REG(`SOC_ZMEAS_SUM_SHIFT_0,top_test_cfg.pads,top_test_cfg.rd_data);

    //Check Device status
      ///-------------to read from the SOC_DEVICE_INT_STATUS_0 reg-------------------//
       `RD_NORMAL_REG(`SOC_DEVICE_INT_STATUS_0,top_test_cfg.pads,top_test_cfg.rd_data);
       if(top_test_cfg.rd_data[1:0] !== 2'b00)
	 `nnc_error("SOC_TEST", "[ERROR][13] Device int status not expected!")
