...
//--------------------------------------------------------------------------------------
// File Name	: soc_zmeas_reset_test.sv                                                   
// Project	: Nanochap BAF4P1                                  		        
// Description	: Testcase soc_zmeas_reset_test                                             
// Designer	: ophina@nanochap.com                                                                 
// Date		: 06-06-2024                                                                     
// Revision	: 0.1 Initial version created by script                                 
--------------------------------------------------------------------------------------*/
`define TESTNAME soc_zmeas_reset_test
`define TESTCFG soc_zmeas_reset_test_cfg
...
class `TESTCFG extends soc_base_test_cfg;
  ...
  function new (string name = "soc_zmeas_reset_test_cfg");
    super.new(name);    
  endfunction: new
  ...
  constraint c_freq_val       { soft freq_val == `ZMEAS_REG_FREQ_VAL_1KHZ; }//freq_val=1KHz
  // top_test_cfg.top_test_cfg.zmeas_freq_sel values
  constraint c_zmeas_freq_sel { soft zmeas_freq_sel == 2'b00; }//freq_val=1KHz
...
class `TESTNAME extends soc_base_test;
  ...
  virtual task main_phase(uvm_phase phase);
    ...
    `nnc_info("SOC_TEST", "soc_zmeas_reset_test start", UVM_LOW)

    // ----------------------------------------------------------------------------------
    // Please add your code of your test here
    // ----------------------------------------------------------------------------------  
    `nnc_info("SOC_TEST",$sformatf("Simulation Begin!!!"),UVM_LOW);
    `nnc_info("SOC_TEST",$sformatf("ZMEAS reset test"),UVM_LOW);
    do_run;
  ...
  virtual task do_run;
   ...
   //Enable ZMEAS
    ...
    #10000000;
    top_test_cfg.wr_data[0] = {top_test_cfg.tx_gain_sel[1],top_test_cfg.calib,top_test_cfg.zmeas_analog_en,top_test_cfg.tx_gain_sel[0],1'b0,top_test_cfg.rx_pga_gain,2'b01};
    //Apply reset after a delay
       `nnc_info("SOC_TEST", "Apply Reset!", UVM_LOW)
    ////------------- to write in to the SOC_ZMEAS_REG_CTRL_0 reg--------------//
       `WR_NORMAL_REG(`SOC_ZMEAS_REG_CTRL_0, top_test_cfg.wr_data[0], top_test_cfg.pads);
    ...
    //Check int status
	  ...
	  `nnc_error("SOC_TEST", "[ERROR][1] zmeas reset failed!")
       else
	  `nnc_info("SOC_TEST", "zmeas reset passed!", UVM_LOW)
    ...
    ///-------------to read from the SOC_ZMEAS_REG_STATUS_1 reg-------------------//
       ...
       if(top_test_cfg.status[8:0] !== 0)
         `nnc_error("SOC_TEST", "[ERROR][4] zmeas_status[8:0] is not expected!")
    ...
    ///-------------to read burst starting from the SOC_ZMEAS_REG_DATAOUT_0 reg-------------------//
       ...
       if(top_test_cfg.zmeas_data !== 32'h0)
         `nnc_error("SOC_TEST", "[ERROR][5] ZMEAS DATA INCORRECT!")
