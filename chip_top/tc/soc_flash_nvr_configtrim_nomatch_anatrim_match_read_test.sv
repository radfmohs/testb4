...
//--------------------------------------------------------------------------------------
// File Name	: soc_flash_nvr_configtrim_nomatch_anatrim_match_read_test.sv                                                   
// Project	: Nanochap ENS1P4                                  		        
// Description	: Testcase soc_flash_nvr_configtrim_nomatch_anatrim_match_read_test                                             
// Designer	: ophina@nanochap.com                                                                 
// Date		: 29-11-1623                                                                     
// Revision	: 0.1 Initial version created by script                                 
--------------------------------------------------------------------------------------*/
`define TESTNAME soc_flash_nvr_configtrim_nomatch_anatrim_match_read_test
`define TESTCFG soc_flash_nvr_configtrim_nomatch_anatrim_match_read_test_cfg
...
class `TESTCFG extends soc_base_test_cfg;
  ...
  function new (string name = "soc_flash_nvr_configtrim_nomatch_anatrim_match_read_test_cfg");
    super.new(name);    
  endfunction: new
...
class `TESTNAME extends soc_base_test;
  ...
  virtual task main_phase(uvm_phase phase);
    ...
    `nnc_info("SOC_TEST", "soc_flash_nvr_configtrim_nomatch_anatrim_match_read_test start", UVM_LOW)

    // ----------------------------------------------------------------------------------
    // Please add your code of your test here
    // ----------------------------------------------------------------------------------  
    `nnc_info("SOC_TEST",$sformatf("Simulation Begin!!!"),UVM_LOW);
    `nnc_info("SOC_TEST",$sformatf("flash nvr read function config trim not match, ana trim match test"),UVM_LOW);
    wait(`RESETN);
    do_run;
