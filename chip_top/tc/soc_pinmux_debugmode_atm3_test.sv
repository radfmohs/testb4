...
class `TESTNAME extends soc_base_test;
  ...
  virtual task main_phase(uvm_phase phase);
    ...
    `nnc_info("SOC_TEST", "soc_pinmux_debugmode_atm3_test start", UVM_LOW)
...
`ifndef FPGA
    ...
    force `SOC_TB.IOBUF_PAD[9:6] = 4'b0100;  
      
    #10000;

    //if(`ANA_TOP.D2A_ECG_DDA_ISEL[1:0] !== 2'b00) `nnc_error("ATM3",$sformatf("default error"));
    ...
    if (top_test_cfg.atm !== (13'b1_0000_0000_0000 >> `SOC_TB.IOBUF_PAD[9:6]-1))
      begin
        `nnc_error("ATM3",$sformatf("ATM[0:12] = %b is not as expectation of ATM = %b", top_test_cfg.atm, (13'b1_0000_0000_0000 >> `SOC_TB.IOBUF_PAD[9:6]-1))); 
      end

    if (`ANA_TOP.D2A_BIST_EN !== 1'b0)
      `nnc_error("ATM3",$sformatf("[ERROR] ANA_TOP.D2A_BIST_EN = %b is not as expectation 1'b0", `ANA_TOP.D2A_BIST_EN));

    if (`ANA_TOP.D2A_ECGCAL_EN !== 1'b0)
      `nnc_error("ATM3",$sformatf("[ERROR] ANA_TOP.D2A_ECGCAL_EN = %b is not as expectation 1'b0", `ANA_TOP.D2A_ECGCAL_EN));

      
    //Check default signal           
    if (`ANA_TOP.D2A_ECG_DDA_ISEL[1:0] !== 2'b0)
      begin
        `nnc_error("ATM3", $sformatf("D2A_ECG_DDA_ISEL[1:0] = %b is not as expectation of 1'b0", `ANA_TOP.D2A_ECG_DDA_ISEL[1:0]))
      end 

    // Checking pin D2A_ECG_DDA_EN
    for (int i=0; i < 100; i++) begin
      force `SOC_TB.IOBUF_PAD[0] = $random;
      #10000;
      rand_bit = `SOC_TB.IOBUF_PAD[0];     
      if (`ANA_TOP.D2A_ECG_DDA_EN !== rand_bit) begin
        `nnc_error("ATM3",$sformatf("[ERROR][%d] D2A_ECG_DDA_EN = %b is not as expectation of rand_bit = %b", i, `ANA_TOP.D2A_ECG_DDA_EN , rand_bit));
      end
    ...
    for (int i=0; i < 100; i++) begin
      force `SOC_TB.IOBUF_PAD[1] = $random;
      #10000;
      rand_bit = `SOC_TB.IOBUF_PAD[1];     
      if (`ANA_TOP.D2A_ECG_PGA_EN !== rand_bit) begin
        `nnc_error("ATM3",$sformatf("[ERROR][%d] D2A_ECG_PGA_EN = %b is not as expectation of rand_bit = %b", i, `ANA_TOP.D2A_ECG_PGA_EN , rand_bit));
      end
    ...
    for (int i=0; i < 100; i++) begin
      force `SOC_TB.IOBUF_PAD[2] = $random;
      #10000;
      rand_bit = `SOC_TB.IOBUF_PAD[2];     
      if (`ANA_TOP.D2A_ECG_LPF_EN !== rand_bit) begin
        `nnc_error("ATM3",$sformatf("[ERROR][%d] D2A_ECG_LPF_EN = %b is not as expectation of rand_bit = %b", i, `ANA_TOP.D2A_ECG_LPF_EN , rand_bit));
      end
    ...
    for (int i=0; i < 100; i++) begin
      force `SOC_TB.IOBUF_PAD[3] = $random;
      #10000;
      rand_bit = `SOC_TB.IOBUF_PAD[3];     
      if (`ANA_TOP.D2A_ECG_DDACHOP_EN !== rand_bit) begin
        `nnc_error("ATM3",$sformatf("[ERROR][%d] D2A_ECG_DDACHOP_EN = %b is not as expectation of rand_bit = %b", i, `ANA_TOP.D2A_ECG_DDACHOP_EN , rand_bit));
      end
    ...
    for (int i=0; i < 100; i++) begin
      force {`SOC_TB.IOBUF_PAD[4],`SOC_TB.IOBUF_PAD[11]} = $random;
      #10000;
      rand_num[1:0] = {`SOC_TB.IOBUF_PAD[4],`SOC_TB.IOBUF_PAD[11]};     
      if (`ANA_TOP.D2A_ECG_DDA_CLKSEL !== rand_num[1:0]) begin
        `nnc_error("ATM3",$sformatf("[ERROR][%d] D2A_ECG_DDA_CLKSEL = %b is not as expectation of rand_num[1:0] = %b", i, `ANA_TOP.D2A_ECG_DDA_CLKSEL , rand_num[1:0]));
      end
      release {`SOC_TB.IOBUF_PAD[4],`SOC_TB.IOBUF_PAD[11]};
      #10000;
    end

    // Checking pin D2A_ECG_PGA_GSEL
    for (int i=0; i < 100; i++) begin
      force {`SOC_TB.IOBUF_PAD[12],`SOC_TB.IOBUF_PAD[13],`SOC_TB.IOBUF_PAD[5]} = $random;
      #10000;
      rand_num[2:0] = {`SOC_TB.IOBUF_PAD[12],`SOC_TB.IOBUF_PAD[13],`SOC_TB.IOBUF_PAD[5]};     
      if (`ANA_TOP.D2A_ECG_PGA_GSEL !== rand_num[2:0]) begin
        `nnc_error("ATM3",$sformatf("[ERROR][%d] D2A_ECG_PGA_GSEL - {`SOC_TB.IOBUF_PAD[12],`SOC_TB.IOBUF_PAD[13],`SOC_TB.IOBUF_PAD[5]} = %b is not as expectation of `ANA_TOP.D2A_ECG_PGA_GSEL = %b", i, {`SOC_TB.IOBUF_PAD[12],`SOC_TB.IOBUF_PAD[13],`SOC_TB.IOBUF_PAD[5]} , `ANA_TOP.D2A_ECG_PGA_GSEL));
      end
      release {`SOC_TB.IOBUF_PAD[12],`SOC_TB.IOBUF_PAD[13],`SOC_TB.IOBUF_PAD[5]};
      #10000;
    end

    // Checking pin D2A_SDM_CLK 
    for (int i=0; i < 100; i++) begin
      force `SOC_TB.IOBUF_PAD[14] = $random;
      #10000;
      rand_bit = `SOC_TB.IOBUF_PAD[14];     
      if (`ANA_TOP.D2A_SDM_CLK !== rand_bit) begin
        `nnc_error("ATM3",$sformatf("[ERROR][%d] D2A_SDM_CLK - `SOC_TB.IOBUF_PAD[14] = %b is not as expectation of `ANA_TOP.D2A_SDM_CLK = %b", i, `SOC_TB.IOBUF_PAD[14] , `ANA_TOP.D2A_SDM_CLK));
      end
      release `SOC_TB.IOBUF_PAD[14];
      #10000;
    end

    // Checking pin D2A_ECG_DDA_GSEL
    for (int i=0; i < 100; i++) begin
      force {`SOC_TB.IOBUF_PAD[15],`SOC_TB.IOBUF_PAD[16],`SOC_TB.IOBUF_PAD[17]} = $random;
      #10000;
      rand_num[2:0] = {`SOC_TB.IOBUF_PAD[15],`SOC_TB.IOBUF_PAD[16],`SOC_TB.IOBUF_PAD[17]};     
      if (`ANA_TOP.D2A_ECG_DDA_GSEL !== rand_num[2:0]) begin
        `nnc_error("ATM3",$sformatf("[ERROR][%d] D2A_ECG_DDA_GSEL - {`SOC_TB.IOBUF_PAD[115],`SOC_TB.IOBUF_PAD[16],`SOC_TB.IOBUF_PAD[17]} = %b is not as expectation of `ANA_TOP.D2A_ECG_DDA_GSEL = %b", i, {`SOC_TB.IOBUF_PAD[15],`SOC_TB.IOBUF_PAD[16],`SOC_TB.IOBUF_PAD[17]} , `ANA_TOP.D2A_ECG_DDA_GSEL));
      end
      release {`SOC_TB.IOBUF_PAD[15],`SOC_TB.IOBUF_PAD[16],`SOC_TB.IOBUF_PAD[17]};
      #10000;
    end
    ...
    for (int i=0; i < 100; i++) begin
      ...
      if (`ANA_TOP.D2A_VCM1P5_BUF_EN !== rand_bit) begin
        `nnc_error("ATM9",$sformatf("[ERROR][%d] D2A_VCM1P5_BUF_EN - `SOC_TB.IOBUF_PAD[18] = %b is not as expectation of `ANA_TOP.D2A_VCM1P5_BUF_EN = %b", i, `SOC_TB.IOBUF_PAD[18] , `ANA_TOP.D2A_VCM1P5_BUF_EN));
      end
