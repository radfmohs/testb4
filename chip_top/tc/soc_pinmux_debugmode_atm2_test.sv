...
class `TESTNAME extends soc_base_test;
  ...
  virtual task main_phase(uvm_phase phase);
    ...
    `nnc_info("SOC_TEST", "soc_pinmux_debugmode_atm2_test start", UVM_LOW)
...
`ifndef FPGA
    ...
    force `SOC_TB.IOBUF_PAD[9:6] = 4'b0011;  
    ...
    if (top_test_cfg.atm !== (13'b1_0000_0000_0000 >> `SOC_TB.IOBUF_PAD[9:6]-1))
      begin
        `nnc_error("ATM2",$sformatf("ATM[0:12] = %b is not as expectation of ATM = %b", top_test_cfg.atm, (13'b1_0000_0000_0000 >> `SOC_TB.IOBUF_PAD[9:6]-1))); 
      end

    if (`ANA_TOP.D2A_BIST_EN !== 1'b0)
      `nnc_error("ATM2",$sformatf("[ERROR] ANA_TOP.D2A_BIST_EN = %b is not as expectation 1'b0", `ANA_TOP.D2A_BIST_EN));

    if (`ANA_TOP.D2A_ECGCAL_EN !== 1'b0)
      `nnc_error("ATM2",$sformatf("[ERROR] ANA_TOP.D2A_ECGCAL_EN = %b is not as expectation 1'b0", `ANA_TOP.D2A_ECGCAL_EN));


    // Checking pin D2A_SDM_VIN_SEL 
    for (int i=0; i < 100; i++) begin
      force {`SOC_TB.IOBUF_PAD[0],`SOC_TB.IOBUF_PAD[2],`SOC_TB.IOBUF_PAD[1]} = $random;
      #10000;
      rand_num[2:0] = {`SOC_TB.IOBUF_PAD[0],`SOC_TB.IOBUF_PAD[2],`SOC_TB.IOBUF_PAD[1]};     
      if (`ANA_TOP.D2A_SDM_VIN_SEL !== rand_num[2:0]) begin
        `nnc_error("ATM2",$sformatf("[ERROR][%d] D2A_SDM_VIN_SEL = %b is not as expectation of rand_num[2:0] = %b", i,`ANA_TOP.D2A_SDM_VIN_SEL , rand_num[2:0]));
      end
      release {`SOC_TB.IOBUF_PAD[0],`SOC_TB.IOBUF_PAD[2],`SOC_TB.IOBUF_PAD[1]};
      #10000;
    end

    // Checking pin D2A_SDM_EN 
    for (int i=0; i < 100; i++) begin
      force `SOC_TB.IOBUF_PAD[3] = $random;
      #10000;
      rand_bit = `SOC_TB.IOBUF_PAD[3];     
      if (`ANA_TOP.D2A_SDM_EN !== rand_bit) begin
        `nnc_error("ATM2",$sformatf("[ERROR][%d] D2A_SDM_EN = %b is not as expectation of rand_bit = %b", i, `ANA_TOP.D2A_SDM_EN , rand_bit));
      end
      release `SOC_TB.IOBUF_PAD[3];
      #10000;
    end

    // Checking pin A2D_SDM_OUT 
    for (int i=0; i < 100; i++) begin
      force `ANA_TOP.A2D_SDM_OUT = $random;
      #10000;
      rand_bit = `ANA_TOP.A2D_SDM_OUT;     
      if (`SOC_TB.IOBUF_PAD[4] !== rand_bit) begin
        `nnc_error("ATM2",$sformatf("[ERROR][%d] A2D_SDM_OUT = %b is not as expectation of rand_bit = %b", i, `ANA_TOP.A2D_SDM_OUT , rand_bit));
      end
    ...
    for (int i=0; i < 100; i++) begin
      force {`SOC_TB.IOBUF_PAD[12],`SOC_TB.IOBUF_PAD[11]} = $random;
      #10000;
      rand_num[1:0] = {`SOC_TB.IOBUF_PAD[12],`SOC_TB.IOBUF_PAD[11]};     
      if (`ANA_TOP.D2A_SDM_BIAS !== rand_num[1:0]) begin
        `nnc_error("ATM2",$sformatf("[ERROR][%d] D2A_SDM_BIAS = %b is not as expectation of rand_num[1:0] = %b", i, `ANA_TOP.D2A_SDM_BIAS , rand_num[1:0]));
      end
      release {`SOC_TB.IOBUF_PAD[12],`SOC_TB.IOBUF_PAD[11]};
      #10000;
    end
    ...
    for (int i=0; i < 100; i++) begin
      ...
      if (`ANA_TOP.D2A_SDM_CLK !== rand_bit) begin
        `nnc_error("ATM2",$sformatf("[ERROR][%d] D2A_SDM_CLK = %b is not as expectation of rand_bit = %b", i, `ANA_TOP.D2A_SDM_CLK , rand_bit));
      end
    ...
    // Checking pin D2A_BG1P2_BUF_EN 
    for (int i=0; i < 100; i++) begin
      force `SOC_TB.IOBUF_PAD[16] = $random;
      #10000;
      rand_bit = `SOC_TB.IOBUF_PAD[16];  
      if (`ANA_TOP.D2A_BG1P2_BUF_EN !== rand_bit) begin
        `nnc_error("ATM9",$sformatf("[ERROR][%d] D2A_BG1P2_BUF_EN = %b is not as expectation of rand_bit = %b", i, `ANA_TOP.D2A_BG1P2_BUF_EN , rand_bit));
      end
