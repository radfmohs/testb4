...
//--------------------------------------------------------------------------------------
// File Name	: soc_flash_nvr_trimmatch_read_test.sv                                                   
// Project	: Nanochap ENS1P4                                  		        
// Description	: Testcase soc_flash_nvr_trimmatch_read_test                                             
// Designer	: ophina@nanochap.com                                                                 
// Date		: 29-11-1623                                                                     
// Revision	: 0.1 Initial version created by script                                 
--------------------------------------------------------------------------------------*/
`define TESTNAME soc_flash_nvr_trimmatch_read_test
`define TESTCFG soc_flash_nvr_trimmatch_read_test_cfg
...
class `TESTCFG extends soc_base_test_cfg;
  ...
  function new (string name = "soc_flash_nvr_trimmatch_read_test_cfg");
    super.new(name);    
  endfunction: new
...
class `TESTNAME extends soc_base_test;
  ...
  virtual task pre_reset_phase(nnc_phase phase);
    ...
    `SPI_SCB_EN = 1'b0;
    #50;
    $readmemh("./inf1_tagmatch.hex", `FLASH_TOP.u_32k.inf1_mem);
    // ==================
    // Scoreboard enables
    // ==================
  ...
  virtual task main_phase(uvm_phase phase);
    ...
    `nnc_info("SOC_TEST", "soc_flash_nvr_trimmatch_read_test start", UVM_LOW)

    // ----------------------------------------------------------------------------------
    // Please add your code of your test here
    // ----------------------------------------------------------------------------------  
    `nnc_info("SOC_TEST",$sformatf("Simulation Begin!!!"),UVM_LOW);
    `nnc_info("SOC_TEST",$sformatf("flash nvr read function trim_tag match"),UVM_LOW);
    wait(`RESETN);
    do_run;
  ...
  virtual task do_run;
    ...
    //   end
       ...
       for(int i=0; i<=7; i++) begin
        ...
        if(top_test_cfg.config_data[i] != (i)) begin
