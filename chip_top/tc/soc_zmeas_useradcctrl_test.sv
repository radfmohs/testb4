...
//--------------------------------------------------------------------------------------
// File Name	: soc_zmeas_useradcctrl_test.sv                                                   
// Project	: Nanochap BAF4P1                                  		        
// Description	: Testcase soc_zmeas_useradcctrl_test                                             
// Designer	: ophina@nanochap.com                                                                 
// Date		: 06-06-2024                                                                     
// Revision	: 0.1 Initial version created by script                                 
--------------------------------------------------------------------------------------*/
`define TESTNAME soc_zmeas_useradcctrl_test
`define TESTCFG soc_zmeas_useradcctrl_test_cfg
...
class `TESTCFG extends soc_base_test_cfg;
  ...
  function new (string name = "soc_zmeas_useradcctrl_test_cfg");
    super.new(name);    
  endfunction: new
  ...
  constraint c_freq_val       { soft freq_val == `ZMEAS_REG_FREQ_VAL_500HZ; }//freq_val=500Hz
...
class `TESTNAME extends soc_base_test;
  ...
  virtual task main_phase(uvm_phase phase);
    ...
    `nnc_info("SOC_TEST", "soc_zmeas_useradcctrl_test start", UVM_LOW)

    // ----------------------------------------------------------------------------------
    // Please add your code of your test here
    // ----------------------------------------------------------------------------------  
    `nnc_info("SOC_TEST",$sformatf("Simulation Begin!!!"),UVM_LOW);
    `nnc_info("SOC_TEST",$sformatf("ZMEAS user controlled ADC test"),UVM_LOW);
    do_run;
   ...
   //Enable ZMEAS
    ...
    ///-------------to read from the SOC_ZMEAS_EN reg-------------------//
	  ...
	  `nnc_error("SOC_TEST", "[ERROR][0] zmeas not enabled!")
       ...
       //Set user control ADC ANA_Z_ADC_DAC_EN_SEL[0]
    ////------------- to write in to the SOC_ANA_Z_ADC_DAC_EN_SEL reg--------------//
       `WR_NORMAL_REG(`SOC_ANA_Z_ADC_DAC_EN_SEL, top_test_cfg.wr_data[0], top_test_cfg.pads);
    ...
    ////------------- to write in to the SOC_ZMEAS_REG_CTRL_1 reg--------------//
       ...
       `nnc_info("SOC_TEST", "*****  Start ADC  *****", UVM_LOW)
       top_test_cfg.wr_data[0] = 8'h01;
   //User Start ADC ANA_Z_ADC_DAC_EN[0]
    ////------------- to write in to the SOC_ANA_Z_ADC_DAC_EN reg--------------//
       `WR_NORMAL_REG(`SOC_ANA_Z_ADC_DAC_EN, top_test_cfg.wr_data[0], top_test_cfg.pads);
     ...
     ////------------- to write in to the SOC_ZMEAS_REG_CTRL_1 reg--------------//
       ...
       #1000000;
       `nnc_info("SOC_TEST", "*****  Stop ADC  *****", UVM_LOW)
       top_test_cfg.wr_data[0] = 8'h00;
   //User Stop ADC ANA_Z_ADC_DAC_EN[0]
    ////------------- to write in to the SOC_ANA_Z_ADC_DAC_EN reg--------------//
       `WR_NORMAL_REG(`SOC_ANA_Z_ADC_DAC_EN, top_test_cfg.wr_data[0], top_test_cfg.pads);
    ...
    //Check int status
       if((top_test_cfg.int_sts[0] === 1'b0) && (`SOC_TB.IOBUF_PAD[4] === 1'b0))
          `nnc_info("SOC_TEST", "zmeas interrupt not occurred!", UVM_LOW)
       else
          `nnc_error("SOC_TEST", "[ERROR][1] Unexpected zmeas interrupt!")
    ...
    ///-------------to read from the SOC_ZMEAS_REG_STATUS_1 reg-------------------//
       ...
       if(top_test_cfg.status[2:0] !== 0) //measurement will be stopped
         `nnc_error("SOC_TEST", "[ERROR][8] zmeas_status[2:0] is not expected!")
       if(`ANA_TOP.D2A_SW_Z_ADC_EN !== 1'b0) //FINAL ADC ENABLE TO ANALOG SHOULD BE STOPPED
         `nnc_error("SOC_TEST", "[ERROR][9] D2A_SW_Z_ADC_EN is not expected!")
