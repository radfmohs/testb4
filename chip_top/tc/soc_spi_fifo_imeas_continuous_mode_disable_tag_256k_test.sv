...
//--------------------------------------------------------------------------------------
// File Name	: soc_spi_fifo_imeas_continuous_mode_disable_tag_256k_test.sv                                                   
// Project	: Nanochap ENS3                                  		        
// Description	: Testcase soc_spi_fifo_imeas_continuous_mode_disable_tag_256k_test                                             
...
// -------------------------------------------------
`define TESTNAME soc_spi_fifo_imeas_continuous_mode_disable_tag_256k_test
`define TESTCFG soc_spi_fifo_imeas_continuous_mode_disable_tag_256k_test_cfg
...
class `TESTCFG extends soc_base_test_cfg;
  ...
  function new (string name = "soc_spi_fifo_imeas_continuous_mode_disable_tag_256k_test_cfg");
    super.new(name);
...
class `TESTNAME extends soc_base_test;
  ...
  virtual task main_phase(uvm_phase phase);
    ...
    `nnc_info("SOC_TEST", "soc_spi_fifo_imeas_continuous_mode_disable_tag_256k_test start", UVM_LOW)
    ...
    `nnc_info(get_full_name(), $sformatf("[DEBUG] - STEP 1"),UVM_LOW);
    ...
    `nnc_info(get_full_name(), $sformatf("[DEBUG] - STEP 2"),UVM_LOW);
    // ----------------------------------------
    // TEST with NO TAGs
    // ----------------------------------------
    ...
    `nnc_info(get_full_name(), $sformatf("[DEBUG] - STEP 3"),UVM_LOW);
...
`endif
    ...
    for (int i = 0; i < loop1; i++) begin
      ...
      `nnc_info(get_full_name(), $sformatf("[DEBUG] - STEP 3.2"),UVM_LOW);
    ...
    `nnc_info(get_full_name(), $sformatf("[DEBUG] - STEP 4"),UVM_LOW);
    for (int i=10; i < 3; i++) begin
      ...
      `nnc_info(get_full_name(), $sformatf("[DEBUG] - STEP 4.1"),UVM_LOW);
      // Disble IMEAS
      wr_data = 8'h00; 
      `WR_RD_CHK_NORMAL_REG(`SOC_IMEAS_EN,wr_data,top_test_cfg.pads,top_test_cfg.mask);
    ...
    // --------------------------------------------------------
    //#10000ns;
    `nnc_info("SOC_TEST", "soc_spi_fifo_imeas_continuous_mode_disable_tag_256k_test end now", UVM_LOW)
