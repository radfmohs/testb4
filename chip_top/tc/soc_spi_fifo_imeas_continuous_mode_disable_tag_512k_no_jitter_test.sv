...
//--------------------------------------------------------------------------------------
// File Name	: soc_spi_fifo_imeas_continuous_mode_disable_tag_512k_no_jitter_test.sv                                                   
// Project	: Nanochap ENS3                                  		        
// Description	: Testcase soc_spi_fifo_imeas_continuous_mode_disable_tag_512k_no_jitter_test                                             
...
// -------------------------------------------------
`define TESTNAME soc_spi_fifo_imeas_continuous_mode_disable_tag_512k_no_jitter_test
`define TESTCFG soc_spi_fifo_imeas_continuous_mode_disable_tag_512k_no_jitter_test_cfg
...
class `TESTCFG extends soc_base_test_cfg;
  ...
  function new (string name = "soc_spi_fifo_imeas_continuous_mode_disable_tag_512k_no_jitter_test_cfg");
    super.new(name);
  ...
  constraint c_spi_clk_jitter         { soft spi_clk_jitter == 0;}

  constraint c_spi_sclk_jitter         { soft spi_sclk_jitter == 0;}

  constraint c_spi_sclk_freq          {soft spi_sclk_freq == 512;}
...
class `TESTNAME extends soc_base_test;
  ...
  virtual task main_phase(uvm_phase phase);
    ...
    `nnc_info("SOC_TEST", "soc_spi_fifo_imeas_continuous_mode_disable_tag_512k_no_jitter_test start", UVM_LOW)
    ...
    // --------------------------------------------------------
    //#10000ns;
    `nnc_info("SOC_TEST", "soc_spi_fifo_imeas_continuous_mode_disable_tag_512k_no_jitter_test end now", UVM_LOW)
...
`endif
    ...
    for (int i = 0; i < loop1; i++) begin
	  ...
	  `nnc_info(get_full_name(), $sformatf("DATA from IMEAS is received at SPI Master through READ REGISTER is VALUE=%h and Expected Value =%h",rd_data_fifo_reg, `SPI_IF_EXP_DATA),UVM_MEDIUM);
        ...
        //$display("[DEBUG] - STEP 4 %d", i);
	`nnc_info(get_full_name(), $sformatf("[DEBUG] - STEP 4"),UVM_MEDIUM);
    
        // Read a burst from FIFO
        no_of_elements = 1;
        rd_data_fifo = new[no_of_elements];
        `RD_BURST_FIFO_NO_TAG(no_of_elements, rd_data_fifo); 
    
        // Expected read pointer 
        rd_counter = rd_counter + no_of_elements;
	  ...
	  `nnc_error(get_full_name(), $sformatf("FIFO_READ_POINTER is CURRENT VALUE=%h != EXPECTED VALUE=%h", rd_data, rd_counter));
        ...
        for (int i = 0; i < no_of_elements; i++) begin
...
/*    
      // Extract from FIFO
      rd_data_imeas2fifo = imeas2fifo_data_mem.pop_front();
      // Compare DATA read in registers and from FIFO data bus
      for (int i = 0; i < no_of_elements; i++) begin
    ...
    //$display("[DEBUG] - STEP 5");
    `nnc_info(get_full_name(), $sformatf("[DEBUG] - STEP 5"),UVM_MEDIUM);
    ...
    for (int i=10; i < 3; i++) begin
      ...
      //$display("[DEBUG] - STEP 5 - %d", i);
      `nnc_info(get_full_name(), $sformatf("[DEBUG] - STEP 5 - %d",i),UVM_MEDIUM);
...
/*      
      ...
      for (int i = 0; i < no_of_elements; i++) begin
          // Extract from FIFO
        rd_data_imeas2fifo = imeas2fifo_data_mem.pop_front();
        if (rd_data_imeas2fifo[15:0] !== rd_data_fifo[no_of_elements - 1 - i]) begin
          $display("[ERROR][7] READ DATA from FIFO Register Element-%d: %h is NOT EQUAL to THE VALUE from IMEAS to FIFO: %h", i, rd_data_fifo[no_of_elements - 1 - i], rd_data_imeas2fifo[15:0]);
        end
      end
    */
      // Get TAGs from Status register
