...
//--------------------------------------------------------------------------------------
// File Name	: soc_spi_fifo_imeas_group_mode_enable_tag_test_clk_config_min.sv                                                   
// Project	: Nanochap ENS3                                  		        
// Description	: Testcase soc_spi_fifo_imeas_group_mode_enable_tag_test_clk_config_min                                             
...
// -------------------------------------------------
`define TESTNAME soc_spi_fifo_imeas_group_mode_enable_tag_test_clk_config_min
`define TESTCFG soc_spi_fifo_imeas_group_mode_enable_tag_test_clk_config_min_cfg
...
class `TESTCFG extends soc_base_test_cfg;
  ...
  function new (string name = "soc_spi_fifo_imeas_group_mode_enable_tag_test_clk_config_min_cfg");
    super.new(name);
  ...
  //constraint c_iclk_sel       { soft iclk_sel inside {[4:7]}; }//make sure to Imeas run with 8Khz to 1kz- slower than SPI imeas adc clk
...
class `TESTNAME extends soc_base_test;
  ...
  virtual task main_phase(uvm_phase phase);
    ...
    `nnc_info("SOC_TEST", "soc_spi_fifo_imeas_group_mode_enable_tag_test_clk_config_min start", UVM_LOW)
...
/*
 ...
 if(rd_data[3] === 1 && rd_data[5] === 1)begin
  ...
  wr_data = 8'h01; 
  `WR_RD_CHK_NORMAL_REG(`SOC_IMEAS_EN,wr_data,top_test_cfg.pads,top_test_cfg.mask);
  `nnc_info(get_full_name(), $sformatf("****** for loop done: %d",i),UVM_LOW);
end

`nnc_info(get_full_name(), $sformatf("****** imeas2fifo_mem_cnt: %d",top_cfg.imeas2fifo_mem_cnt),UVM_LOW);
loop2 = loop1 + 2;
...
/*  
  ...
  for (int i = 0; i < top_test_cfg.no_of_elements; i++) begin
    ...
    rd_data_imeas2fifo = top_cfg.imeas2fifo_data_mem.pop_front();
    if (rd_data_imeas2fifo[17:0] !== rd_data_fifo_tags[top_test_cfg.no_of_elements - 1 - i][17:0]) begin
      //$display("[ERROR][6] READ DATA from FIFO Element-%d: %h is NOT EQUAL to THE VALUE from IMEAS to FIFO: %h", i, rd_data_imeas2fifo[17:0], rd_data_fifo_tags[top_test_cfg.no_of_elements - 1 - i][17:0]);
      `nnc_error(get_full_name(), $sformatf("READ DATA from FIFO Element-%d: %h is NOT EQUAL to THE VALUE from IMEAS to FIFO: %h", i, rd_data_fifo_tags[top_test_cfg.no_of_elements - 1 - i][17:0], rd_data_imeas2fifo[17:0]));
    end
  end */
 ...
 // check full and almost full status and clear if asserted
 `nnc_info("SOC_TEST", "check the almost full and fifo_full", UVM_LOW)
 `RD_NORMAL_REG(`SOC_FIFO_STATUS_REG,top_test_cfg.pads,rd_data);
 if(rd_data[3] === 1 && rd_data[5] === 1)begin
   `nnc_info("SOC_TEST", "will clear the almost full and fifo_full", UVM_LOW)
   wr_data = 8'h28; // clearing almost_full and fifo_full status 
  ...
  `WR_RD_CHK_NORMAL_REG(`SOC_IMEAS_EN,wr_data,top_test_cfg.pads,top_test_cfg.mask);
    ...
    // --------------------------------------------------------
    //#10000ns;
    `nnc_info("SOC_TEST", "soc_spi_fifo_imeas_group_mode_enable_tag_test_clk_config_min end now", UVM_LOW)
